...
RealTimer kernelTimerStress;
RealTimer kernelTimerVelocity;
...
void init_cl(int *deviceID) 
{
    ...
    Init(&kernelTimerStress);
    Init(&kernelTimerVelocity);
	Init(&h2dTimerStress);
    ...
    _cl_context = clCreateContext(NULL, 1, &_cl_firstDevice, NULL, NULL, &errNum);
    if(errNum != CL_SUCCESS)
    {
    ...
    if(_cl_commandQueue == NULL)
    {
        fprintf(stderr, "Failed to create commandQueue for the default device!\n");
    }
    ...
    // -cl-mad-enable
    // -cl-opt-disable
    errNum = clBuildProgram(_cl_program, 1, &_cl_firstDevice, "", NULL, NULL);
    //errNum = clBuildProgram(_cl_program, 1, &_cl_firstDevice, "-cl-opt-disable", NULL, NULL);
    if(errNum != CL_SUCCESS)
    {
        char buildLog[16384];
        clGetProgramBuildInfo(_cl_program, _cl_firstDevice, CL_PROGRAM_BUILD_LOG, sizeof(buildLog), buildLog, NULL);
...
void free_device_memC_opencl(int *lbx, int *lby)
{
	...
	Print(&kernelTimerStress, "Stress Kernel 1 Time");
	Print(&kernelTimerVelocity, "Velocity Kernel 1 Time");
					...
					int   *nll)
{
	...
	//for inner_I
    nd1_velD = clCreateBuffer(_cl_context, CL_MEM_READ_WRITE, sizeof(int) * 18, NULL, NULL);
    nd1_txyD = clCreateBuffer(_cl_context, CL_MEM_READ_WRITE, sizeof(int) * 18, NULL, NULL);
    nd1_txzD = clCreateBuffer(_cl_context, CL_MEM_READ_WRITE, sizeof(int) * 18, NULL, NULL);
    nd1_tyyD = clCreateBuffer(_cl_context, CL_MEM_READ_WRITE, sizeof(int) * 18, NULL, NULL);
    nd1_tyzD = clCreateBuffer(_cl_context, CL_MEM_READ_WRITE, sizeof(int) * 18, NULL, NULL);
    ...
    rhoD = clCreateBuffer(_cl_context, CL_MEM_READ_WRITE, sizeof(float) * (*nmat), NULL, NULL);
    CHECK_NULL_ERROR(rhoD, "rhoD");

    drvh1D = clCreateBuffer(_cl_context, CL_MEM_READ_WRITE, sizeof(float) * (*mw1_pml1) * 2, NULL, NULL);
    drti1D = clCreateBuffer(_cl_context, CL_MEM_READ_WRITE, sizeof(float) * (*mw1_pml1) * 2, NULL, NULL);
    drth1D = clCreateBuffer(_cl_context, CL_MEM_READ_WRITE, sizeof(float) * (*mw1_pml1) * 2, NULL, NULL);
    ...
    if (lbx[1] >= lbx[0])
	{
        damp1_xD = clCreateBuffer(_cl_context, CL_MEM_READ_WRITE, sizeof(float) * (*nztop + 1) * (*nytop) * (lbx[1] - lbx[0] + 1), NULL, NULL);
        CHECK_NULL_ERROR(damp1_xD, "damp1_xD");
	}

	if (lby[1] >= lby[0])
	{
        damp1_yD = clCreateBuffer(_cl_context, CL_MEM_READ_WRITE, sizeof(float) * (*nztop + 1) * (*nxtop) * (lby[1] - lby[0] + 1), NULL, NULL);
        CHECK_NULL_ERROR(damp1_yD, "damp1_yD");
	}

    idmat1D = clCreateBuffer(_cl_context, CL_MEM_READ_WRITE, sizeof(int) * (*nztop + 2) * (*nxtop + 1) * (*nytop + 1), NULL, NULL);
    CHECK_NULL_ERROR(idmat1D, "idmat1D");
    dxi1D = clCreateBuffer(_cl_context, CL_MEM_READ_WRITE, sizeof(float) * 4 * (*nxtop), NULL, NULL);
    dyi1D = clCreateBuffer(_cl_context, CL_MEM_READ_WRITE, sizeof(float) * 4 * (*nytop), NULL, NULL);
    dzi1D = clCreateBuffer(_cl_context, CL_MEM_READ_WRITE, sizeof(float) * 4 * (*nztop + 1), NULL, NULL);
    dxh1D = clCreateBuffer(_cl_context, CL_MEM_READ_WRITE, sizeof(float) * 4 * (*nxtop), NULL, NULL);
    dyh1D = clCreateBuffer(_cl_context, CL_MEM_READ_WRITE, sizeof(float) * 4 * (*nytop), NULL, NULL);
    dzh1D = clCreateBuffer(_cl_context, CL_MEM_READ_WRITE, sizeof(float) * 4 * (*nztop + 1), NULL, NULL);
	...
	clamdaD = clCreateBuffer(_cl_context, CL_MEM_READ_WRITE, sizeof(float) * (*nmat), NULL, NULL);
	cmuD = clCreateBuffer(_cl_context, CL_MEM_READ_WRITE, sizeof(float) * (*nmat), NULL, NULL);
	epdtD = clCreateBuffer(_cl_context, CL_MEM_READ_WRITE, sizeof(float) * (*nll), NULL, NULL);
	qwpD = clCreateBuffer(_cl_context, CL_MEM_READ_WRITE, sizeof(float) * (*nmat), NULL, NULL);
	qwsD = clCreateBuffer(_cl_context, CL_MEM_READ_WRITE, sizeof(float) * (*nmat), NULL, NULL);
	qwt1D = clCreateBuffer(_cl_context, CL_MEM_READ_WRITE, sizeof(float) * (*nll), NULL, NULL);
	qwt2D = clCreateBuffer(_cl_context, CL_MEM_READ_WRITE, sizeof(float) * (*nll), NULL, NULL);
...
//for inner_II-----------------------------------------------------------------------------------------
    nd2_velD = clCreateBuffer(_cl_context, CL_MEM_READ_WRITE, sizeof(int) * 18, NULL, NULL);
    nd2_txyD = clCreateBuffer(_cl_context, CL_MEM_READ_WRITE, sizeof(int) * 18, NULL, NULL);
    nd2_txzD = clCreateBuffer(_cl_context, CL_MEM_READ_WRITE, sizeof(int) * 18, NULL, NULL); 
    nd2_tyyD = clCreateBuffer(_cl_context, CL_MEM_READ_WRITE, sizeof(int) * 18, NULL, NULL);
    nd2_tyzD = clCreateBuffer(_cl_context, CL_MEM_READ_WRITE, sizeof(int) * 18, NULL, NULL);
    ...
    drvh2D = clCreateBuffer(_cl_context, CL_MEM_READ_WRITE, sizeof(float) * (*mw2_pml1) * 2, NULL, NULL);
    drti2D = clCreateBuffer(_cl_context, CL_MEM_READ_WRITE, sizeof(float) * (*mw2_pml1) * 2, NULL, NULL);
    drth2D = clCreateBuffer(_cl_context, CL_MEM_READ_WRITE, sizeof(float) * (*mw2_pml1) * 2, NULL, NULL);
    CHECK_NULL_ERROR(drvh2D, "drvh2D");
    CHECK_NULL_ERROR(drti2D, "drti2D");
    CHECK_NULL_ERROR(drth2D, "drth2D");

    idmat2D = clCreateBuffer(_cl_context, CL_MEM_READ_WRITE, sizeof(int) * (*nzbtm + 1) * (*nxbtm + 1) * (*nybtm + 1), NULL, NULL);
    CHECK_NULL_ERROR(idmat2D, "idmat2D");
	
	if (lbx[1] >= lbx[0])
	{
        damp2_xD = clCreateBuffer(_cl_context, CL_MEM_READ_WRITE, sizeof(float) * (*nzbtm) * (*nybtm) * (lbx[1] - lbx[0] + 1), NULL, NULL);
        CHECK_NULL_ERROR(damp2_xD, "damp2_xD");
	}

	if (lby[1] >= lby[0])
	{
        damp2_yD = clCreateBuffer(_cl_context, CL_MEM_READ_WRITE, sizeof(float) * (*nzbtm) * (*nxbtm) * (lby[1] - lby[0] + 1), NULL, NULL);
        CHECK_NULL_ERROR(damp2_yD, "damp2_yD");
	}
    damp2_zD = clCreateBuffer(_cl_context, CL_MEM_READ_WRITE, sizeof(float) * (*nxbtm) * (*nybtm), NULL, NULL);
    CHECK_NULL_ERROR(damp2_zD, "damp2_zD");

    dxi2D = clCreateBuffer(_cl_context, CL_MEM_READ_WRITE, sizeof(float) * 4 * (*nxbtm), NULL, NULL);
    dyi2D = clCreateBuffer(_cl_context, CL_MEM_READ_WRITE, sizeof(float) * 4 * (*nybtm), NULL, NULL);
    dzi2D = clCreateBuffer(_cl_context, CL_MEM_READ_WRITE, sizeof(float) * 4 * (*nzbtm), NULL, NULL);
    dxh2D = clCreateBuffer(_cl_context, CL_MEM_READ_WRITE, sizeof(float) * 4 * (*nxbtm), NULL, NULL);
    dyh2D = clCreateBuffer(_cl_context, CL_MEM_READ_WRITE, sizeof(float) * 4 * (*nybtm), NULL, NULL);
    dzh2D = clCreateBuffer(_cl_context, CL_MEM_READ_WRITE, sizeof(float) * 4 * (*nzbtm), NULL, NULL);
						  ...
						  int   *nzbm1)
{
	...
	//for inner_I
	errNum = clEnqueueWriteBuffer(_cl_commandQueue, nd1_velD, CL_TRUE, 0, sizeof(int) * 18, nd1_vel, 0, NULL, NULL);
	CHECK_ERROR(errNum, "InputDataCopyHostToDevice1, nd1_vel");

	errNum = clEnqueueWriteBuffer(_cl_commandQueue, rhoD, CL_TRUE, 0, sizeof(float) * (*nmat), rho, 0, NULL, NULL);
	CHECK_ERROR(errNum, "InputDataCopyHostToDevice1, rho");
	errNum = clEnqueueWriteBuffer(_cl_commandQueue, drvh1D, CL_TRUE, 0, sizeof(float) * (*mw1_pml1) * 2, drvh1, 0, NULL, NULL);
	CHECK_ERROR(errNum, "InputDataCopyHostToDevice1, drvh1");
	errNum = clEnqueueWriteBuffer(_cl_commandQueue, drti1D, CL_TRUE, 0, sizeof(float) * (*mw1_pml1) * 2, drti1, 0, NULL, NULL);
	CHECK_ERROR(errNum, "InputDataCopyHostToDevice1, drti1");

	if (lbx[1] >= lbx[0])
	{
		errNum = clEnqueueWriteBuffer(_cl_commandQueue, damp1_xD, CL_TRUE, 0, sizeof(float) * (*nztop + 1) * (*nytop) * (lbx[1] - lbx[0] + 1), damp1_x, 0, NULL, NULL);
		CHECK_ERROR(errNum, "InputDataCopyHostToDevice1, damp1_x");
	}

	if (lby[1] >= lby[0])
	{
		errNum = clEnqueueWriteBuffer(_cl_commandQueue, damp1_yD, CL_TRUE, 0, sizeof(float) * (*nztop + 1) * (*nxtop) * (lby[1] - lby[0] + 1), damp1_y, 0, NULL, NULL);
		CHECK_ERROR(errNum, "InputDataCopyHostToDevice1, damp1_y");
	}

	errNum = clEnqueueWriteBuffer(_cl_commandQueue, idmat1D, CL_TRUE, 0, sizeof(int) * (*nztop + 2) * (*nxtop + 1) * (*nytop + 1), idmat1, 0, NULL, NULL);
	CHECK_ERROR(errNum, "InputDataCopyHostToDevice1, idmat1");
	errNum = clEnqueueWriteBuffer(_cl_commandQueue, dxi1D, CL_TRUE, 0, sizeof(float) * 4 * (*nxtop), dxi1, 0, NULL, NULL);
	CHECK_ERROR(errNum, "InputDataCopyHostToDevice1, dxi1");
	errNum = clEnqueueWriteBuffer(_cl_commandQueue, dyi1D, CL_TRUE, 0, sizeof(float) * 4 * (*nytop), dyi1, 0, NULL, NULL);
	CHECK_ERROR(errNum, "InputDataCopyHostToDevice1, dyi1");
	errNum = clEnqueueWriteBuffer(_cl_commandQueue, dzi1D, CL_TRUE, 0, sizeof(float) * 4 * (*nztop + 1), dzi1, 0, NULL, NULL);
	CHECK_ERROR(errNum, "InputDataCopyHostToDevice1, dzi1");
	errNum = clEnqueueWriteBuffer(_cl_commandQueue, dxh1D, CL_TRUE, 0, sizeof(float) * 4 * (*nxtop), dxh1, 0, NULL, NULL);
	CHECK_ERROR(errNum, "InputDataCopyHostToDevice1, dxh1");
	errNum = clEnqueueWriteBuffer(_cl_commandQueue, dyh1D, CL_TRUE, 0, sizeof(float) * 4 * (*nytop), dyh1, 0, NULL, NULL);
	CHECK_ERROR(errNum, "InputDataCopyHostToDevice1, dyh1");
	errNum = clEnqueueWriteBuffer(_cl_commandQueue, dzh1D, CL_TRUE, 0, sizeof(float) * 4 * (*nztop + 1), dzh1, 0, NULL, NULL);
	CHECK_ERROR(errNum, "InputDataCopyHostToDevice1, dzh1");

	errNum = clEnqueueWriteBuffer(_cl_commandQueue, t1xxD, CL_TRUE, 0, sizeof(float) * (*nztop) * (*nxtop + 3) * (*nytop), t1xx, 0, NULL, NULL);
	CHECK_ERROR(errNum, "InputDataCopyHostToDevice1, t1xx");
	errNum = clEnqueueWriteBuffer(_cl_commandQueue, t1xyD, CL_TRUE, 0, sizeof(float) * (*nztop) * (*nxtop + 3) * (*nytop + 3), t1xy, 0, NULL, NULL);
	CHECK_ERROR(errNum, "InputDataCopyHostToDevice1, t1xy");
	errNum = clEnqueueWriteBuffer(_cl_commandQueue, t1xzD, CL_TRUE, 0, sizeof(float) * (*nztop + 1) * (*nxtop + 3) * (*nytop), t1xz, 0, NULL, NULL);
	CHECK_ERROR(errNum, "InputDataCopyHostToDevice1, t1xz");
	errNum = clEnqueueWriteBuffer(_cl_commandQueue, t1yyD, CL_TRUE, 0, sizeof(float) * (*nztop) * (*nxtop) * (*nytop + 3), t1yy, 0, NULL, NULL);
	CHECK_ERROR(errNum, "InputDataCopyHostToDevice1, t1yy");
	errNum = clEnqueueWriteBuffer(_cl_commandQueue, t1yzD, CL_TRUE, 0, sizeof(float) * (*nztop + 1) * (*nxtop) * (*nytop + 3), t1yz, 0, NULL, NULL);
	CHECK_ERROR(errNum, "InputDataCopyHostToDevice1, t1yz");
	errNum = clEnqueueWriteBuffer(_cl_commandQueue, t1zzD, CL_TRUE, 0, sizeof(float) * (*nztop) * (*nxtop) * (*nytop), t1zz, 0, NULL, NULL);
	CHECK_ERROR(errNum, "InputDataCopyHostToDevice1, t1zz");

	if (lbx[1] >= lbx[0])
	{
		nv2 = (lbx[1] - lbx[0] + 1) * (*mw1_pml);
		errNum = clEnqueueWriteBuffer(_cl_commandQueue, v1x_pxD, CL_TRUE, 0, sizeof(float) * (*nztop) * nv2 * (*nytop), v1x_px, 0, NULL, NULL);
		CHECK_ERROR(errNum, "outputDataCopyHostToDevice1, v1x_px");
		errNum = clEnqueueWriteBuffer(_cl_commandQueue, v1y_pxD, CL_TRUE, 0, sizeof(float) * (*nztop) * nv2 * (*nytop), v1y_px, 0, NULL, NULL);
		CHECK_ERROR(errNum, "outputDataCopyHostToDevice1, v1y_px");
		errNum = clEnqueueWriteBuffer(_cl_commandQueue, v1z_pxD, CL_TRUE, 0, sizeof(float) * (*nztop) * nv2 * (*nytop), v1z_px, 0, NULL, NULL);
	...
	{
		nv2 = (lby[1] - lby[0] + 1) * (*mw1_pml);
		errNum = clEnqueueWriteBuffer(_cl_commandQueue, v1x_pyD, CL_TRUE, 0, sizeof(float) * (*nztop) * (*nxtop) * nv2, v1x_py, 0, NULL, NULL);
		CHECK_ERROR(errNum, "outputDataCopyHostToDevice1, v1x_py");
		errNum = clEnqueueWriteBuffer(_cl_commandQueue, v1y_pyD, CL_TRUE, 0, sizeof(float) * (*nztop) * (*nxtop) * nv2, v1y_py, 0, NULL, NULL);
		CHECK_ERROR(errNum, "outputDataCopyHostToDevice1, v1y_py");
		errNum = clEnqueueWriteBuffer(_cl_commandQueue, v1z_pyD, CL_TRUE, 0, sizeof(float) * (*nztop) * (*nxtop) * nv2, v1z_py, 0, NULL, NULL);
		CHECK_ERROR(errNum, "outputDataCopyHostToDevice1, v1z_py");
	}


	//for inner_II
	errNum = clEnqueueWriteBuffer(_cl_commandQueue, nd2_velD, CL_TRUE, 0, sizeof(int) * 18, nd2_vel, 0, NULL, NULL);
	CHECK_ERROR(errNum, "InputDataCopyHostToDevice1, nd2_vel");

	errNum = clEnqueueWriteBuffer(_cl_commandQueue, drvh2D, CL_TRUE, 0, sizeof(float) * (*mw2_pml1) * 2, drvh2, 0, NULL, NULL);
	CHECK_ERROR(errNum, "InputDataCopyHostToDevice1, drvh2");
	errNum = clEnqueueWriteBuffer(_cl_commandQueue, drti2D, CL_TRUE, 0, sizeof(float) * (*mw2_pml1) * 2, drti2, 0, NULL, NULL);
	CHECK_ERROR(errNum, "InputDataCopyHostToDevice1, drti2");

	errNum = clEnqueueWriteBuffer(_cl_commandQueue, idmat2D, CL_TRUE, 0, sizeof(int) * (*nzbtm + 1) * (*nxbtm + 1) * (*nybtm +1), idmat2, 0, NULL, NULL);
	CHECK_ERROR(errNum, "InputDataCopyHostToDevice1, idmat2");
	
	if (lbx[1] >= lbx[0])
	{
		errNum = clEnqueueWriteBuffer(_cl_commandQueue, damp2_xD, CL_TRUE, 0, sizeof(float) * (*nzbtm) * (*nybtm) * (lbx[1] - lbx[0] + 1), damp2_x, 0, NULL, NULL);
        CHECK_ERROR(errNum, "InputDataCopyHostToDevice1, damp2_x");
	}

	if (lby[1] >= lby[0])
	{
		errNum = clEnqueueWriteBuffer(_cl_commandQueue, damp2_yD, CL_TRUE, 0, sizeof(float) * (*nzbtm) * (*nxbtm) * (lby[1] - lby[0] + 1), damp2_y, 0, NULL, NULL);
        CHECK_ERROR(errNum, "InputDataCopyHostToDevice1, damp2_y");
	}
	errNum = clEnqueueWriteBuffer(_cl_commandQueue, damp2_zD, CL_TRUE, 0, sizeof(float) * (*nxbtm) * (*nybtm), damp2_z, 0, NULL, NULL);
	CHECK_ERROR(errNum, "InputDataCopyHostToDevice1, damp2_z");

	errNum = clEnqueueWriteBuffer(_cl_commandQueue, dxi2D, CL_TRUE, 0, sizeof(float) * 4 * (*nxbtm), dxi2, 0, NULL, NULL);
	CHECK_ERROR(errNum, "InputDataCopyHostToDevice1, dxi2");
	errNum = clEnqueueWriteBuffer(_cl_commandQueue, dyi2D, CL_TRUE, 0, sizeof(float) * 4 * (*nybtm), dyi2, 0, NULL, NULL);
	CHECK_ERROR(errNum, "InputDataCopyHostToDevice1, dyi2");
	errNum = clEnqueueWriteBuffer(_cl_commandQueue, dzi2D, CL_TRUE, 0, sizeof(float) * 4 * (*nzbtm), dzi2, 0, NULL, NULL);
	CHECK_ERROR(errNum, "InputDataCopyHostToDevice1, dzi2");
	errNum = clEnqueueWriteBuffer(_cl_commandQueue, dxh2D, CL_TRUE, 0, sizeof(float) * 4 * (*nxbtm), dxh2, 0, NULL, NULL);
	CHECK_ERROR(errNum, "InputDataCopyHostToDevice1, dxh2");
	errNum = clEnqueueWriteBuffer(_cl_commandQueue, dyh2D, CL_TRUE, 0, sizeof(float) * 4 * (*nybtm), dyh2, 0, NULL, NULL);
	CHECK_ERROR(errNum, "InputDataCopyHostToDevice1, dyh2");
	errNum = clEnqueueWriteBuffer(_cl_commandQueue, dzh2D, CL_TRUE, 0, sizeof(float) * 4 * (*nzbtm), dzh2, 0, NULL, NULL);
	CHECK_ERROR(errNum, "InputDataCopyHostToDevice1, dzh2");

	errNum = clEnqueueWriteBuffer(_cl_commandQueue, t2xxD, CL_TRUE, 0, sizeof(float) * (*nzbtm) * (*nxbtm + 3) * (*nybtm), t2xx, 0, NULL, NULL);
	CHECK_ERROR(errNum, "InputDataCopyHostToDevice1, t2xx");
	errNum = clEnqueueWriteBuffer(_cl_commandQueue, t2xyD, CL_TRUE, 0, sizeof(float) * (*nzbtm) * (*nxbtm + 3) * (*nybtm + 3), t2xy, 0, NULL, NULL);
	CHECK_ERROR(errNum, "InputDataCopyHostToDevice1, t2xy");
	errNum = clEnqueueWriteBuffer(_cl_commandQueue, t2xzD, CL_TRUE, 0, sizeof(float) * (*nzbtm + 1) * (*nxbtm + 3) * (*nybtm), t2xz, 0, NULL, NULL);
	CHECK_ERROR(errNum, "InputDataCopyHostToDevice1, t2xz");
	errNum = clEnqueueWriteBuffer(_cl_commandQueue, t2yyD, CL_TRUE, 0, sizeof(float) * (*nzbtm) * (*nxbtm) * (*nybtm + 3), t2yy, 0, NULL, NULL);
	CHECK_ERROR(errNum, "InputDataCopyHostToDevice1, t2yy");
	errNum = clEnqueueWriteBuffer(_cl_commandQueue, t2yzD, CL_TRUE, 0, sizeof(float) * (*nzbtm + 1) * (*nxbtm) * (*nybtm + 3), t2yz, 0, NULL, NULL);
	CHECK_ERROR(errNum, "InputDataCopyHostToDevice1, t2yz");
	errNum = clEnqueueWriteBuffer(_cl_commandQueue, t2zzD, CL_TRUE, 0, sizeof(float) * (*nzbtm + 1) * (*nxbtm) * (*nybtm), t2zz, 0, NULL, NULL);
	CHECK_ERROR(errNum, "InputDataCopyHostToDevice1, t2zz");

	if (lbx[1] >= lbx[0])
	{
		nv2 = (lbx[1] - lbx[0] + 1) * (*mw2_pml);
		errNum = clEnqueueWriteBuffer(_cl_commandQueue, v2x_pxD, CL_TRUE, 0, sizeof(float) * (*nzbtm) * nv2 * (*nybtm), v2x_px, 0, NULL, NULL);
		CHECK_ERROR(errNum, "outputDataCopyHostToDevice1, v2x_px");
		errNum = clEnqueueWriteBuffer(_cl_commandQueue, v2y_pxD, CL_TRUE, 0, sizeof(float) * (*nzbtm) * nv2 * (*nybtm), v2y_px, 0, NULL, NULL);
		CHECK_ERROR(errNum, "outputDataCopyHostToDevice1, v2y_px");
		errNum = clEnqueueWriteBuffer(_cl_commandQueue, v2z_pxD, CL_TRUE, 0, sizeof(float) * (*nzbtm) * nv2 * (*nybtm), v2z_px, 0, NULL, NULL);
	...
	{
		nv2 = (lby[1] - lby[0] + 1) * (*mw2_pml);
		errNum = clEnqueueWriteBuffer(_cl_commandQueue, v2x_pyD, CL_TRUE, 0, sizeof(float) * (*nzbtm) * (*nxbtm) * nv2, v2x_py, 0, NULL, NULL);
		CHECK_ERROR(errNum, "outputDataCopyHostToDevice1, v2x_py");
		errNum = clEnqueueWriteBuffer(_cl_commandQueue, v2y_pyD, CL_TRUE, 0, sizeof(float) * (*nzbtm) * (*nxbtm) * nv2, v2y_py, 0, NULL, NULL);
		CHECK_ERROR(errNum, "outputDataCopyHostToDevice1, v2y_py");
		errNum = clEnqueueWriteBuffer(_cl_commandQueue, v2z_pyD, CL_TRUE, 0, sizeof(float) * (*nzbtm) * (*nxbtm) * nv2, v2z_py, 0, NULL, NULL);
		CHECK_ERROR(errNum, "outputDataCopyHostToDevice1, v2z_py");
	}

	errNum = clEnqueueWriteBuffer(_cl_commandQueue, v2x_pzD, CL_TRUE, 0, sizeof(float) * (*mw2_pml) * (*nxbtm) * (*nybtm), v2x_pz, 0, NULL, NULL);
	CHECK_ERROR(errNum, "outputDataCopyHostToDevice1, v2x_pz");
	errNum = clEnqueueWriteBuffer(_cl_commandQueue, v2y_pzD, CL_TRUE, 0, sizeof(float) * (*mw2_pml) * (*nxbtm) * (*nybtm), v2y_pz, 0, NULL, NULL);
	CHECK_ERROR(errNum, "outputDataCopyHostToDevice1, v2y_pz");
	errNum = clEnqueueWriteBuffer(_cl_commandQueue, v2z_pzD, CL_TRUE, 0, sizeof(float) * (*mw2_pml) * (*nxbtm) * (*nybtm), v2z_pz, 0, NULL, NULL);
						  ...
						  int   *nll)
{
	...
	//for inner_I
	errNum = clEnqueueWriteBuffer(_cl_commandQueue, nd1_txyD, CL_TRUE, 0, sizeof(int) * 18, nd1_txy, 0, NULL, NULL);
	CHECK_ERROR(errNum, "InputDataCopyHostToDevice, nd1_txy");
	errNum = clEnqueueWriteBuffer(_cl_commandQueue, nd1_txzD, CL_TRUE, 0, sizeof(int) * 18, nd1_txz, 0, NULL, NULL);
	CHECK_ERROR(errNum, "InputDataCopyHostToDevice, nd1_txz");
	errNum = clEnqueueWriteBuffer(_cl_commandQueue, nd1_tyyD, CL_TRUE, 0, sizeof(int) * 18, nd1_tyy, 0, NULL, NULL);
	CHECK_ERROR(errNum, "InputDataCopyHostToDevice, nd1_tyy");
	errNum = clEnqueueWriteBuffer(_cl_commandQueue, nd1_tyzD, CL_TRUE, 0, sizeof(int) * 18, nd1_tyz, 0, NULL, NULL);
	CHECK_ERROR(errNum, "InputDataCopyHostToDevice, nd1_tyz");

	errNum = clEnqueueWriteBuffer(_cl_commandQueue, drti1D, CL_TRUE, 0, sizeof(float) * (*mw1_pml1) * 2, drti1, 0, NULL, NULL);
	CHECK_ERROR(errNum, "InputDataCopyHostToDevice, drti1");
	errNum = clEnqueueWriteBuffer(_cl_commandQueue, drth1D, CL_TRUE, 0, sizeof(float) * (*mw1_pml1) * 2, drth1, 0, NULL, NULL);
	...
	errNum = clEnqueueWriteBuffer(_cl_commandQueue, v1xD, CL_TRUE, 0, sizeof(float) * (*nztop + 2) * (*nxtop + 3) * (*nytop + 3), v1x, 0, NULL, NULL);
	CHECK_ERROR(errNum, "InputDataCopyHostToDevice, v1x");
	errNum = clEnqueueWriteBuffer(_cl_commandQueue, v1yD, CL_TRUE, 0, sizeof(float) * (*nztop + 2) * (*nxtop + 3) * (*nytop + 3), v1y, 0, NULL, NULL);
	CHECK_ERROR(errNum, "InputDataCopyHostToDevice, v1y");
	errNum = clEnqueueWriteBuffer(_cl_commandQueue, v1zD, CL_TRUE, 0, sizeof(float) * (*nztop + 2) * (*nxtop + 3) * (*nytop + 3), v1z, 0, NULL, NULL);
	...
	if (lbx[1] >= lbx[0])
	{
		...
		errNum = clEnqueueWriteBuffer(_cl_commandQueue, t1xx_pxD, CL_TRUE, 0, sizeof(float) * (*nztop) * (nti) * (*nytop), t1xx_px, 0, NULL, NULL);
        CHECK_ERROR(errNum, "InputDataCopyHostToDevice, t1xx_px");
		errNum = clEnqueueWriteBuffer(_cl_commandQueue, t1xy_pxD, CL_TRUE, 0, sizeof(float) * (*nztop) * nth * (*nytop), t1xy_px, 0, NULL, NULL);
        CHECK_ERROR(errNum, "InputDataCopyHostToDevice, t1xy_px");
		errNum = clEnqueueWriteBuffer(_cl_commandQueue, t1xz_pxD, CL_TRUE, 0, sizeof(float) * (*nztop+1) * nth * (*nytop), t1xz_px, 0, NULL, NULL);
        CHECK_ERROR(errNum, "InputDataCopyHostToDevice, t1xz_px");
		errNum = clEnqueueWriteBuffer(_cl_commandQueue, t1yy_pxD, CL_TRUE, 0, sizeof(float) * (*nztop) * nti * (*nytop), t1yy_px, 0, NULL, NULL);
        CHECK_ERROR(errNum, "InputDataCopyHostToDevice, t1yy_px");
		errNum = clEnqueueWriteBuffer(_cl_commandQueue, qt1xx_pxD, CL_TRUE, 0, sizeof(float) * (*nztop) * (nti) * (*nytop), qt1xx_px, 0, NULL, NULL);
        CHECK_ERROR(errNum, "InputDataCopyHostToDevice, qt1xx_px");
		errNum = clEnqueueWriteBuffer(_cl_commandQueue, qt1xy_pxD, CL_TRUE, 0, sizeof(float) * (*nztop) * nth * (*nytop), qt1xy_px, 0, NULL, NULL);
        CHECK_ERROR(errNum, "InputDataCopyHostToDevice, qt1xy_px");
		errNum = clEnqueueWriteBuffer(_cl_commandQueue, qt1xz_pxD, CL_TRUE, 0, sizeof(float) * (*nztop+1) * nth * (*nytop), qt1xz_px, 0, NULL, NULL);
        CHECK_ERROR(errNum, "InputDataCopyHostToDevice, qt1xz_px");
		errNum = clEnqueueWriteBuffer(_cl_commandQueue, qt1yy_pxD, CL_TRUE, 0, sizeof(float) * (*nztop) * nti * (*nytop), qt1yy_px, 0, NULL, NULL);
	...
	if (lby[1] >= lby[0])
	{
		nti = (lby[1] - lby[0] + 1) * (*mw1_pml) + lby[1];
		nth = (lby[1] - lby[0] + 1) * (*mw1_pml) + 1 - lby[0];
		errNum = clEnqueueWriteBuffer(_cl_commandQueue, t1xx_pyD, CL_TRUE, 0, sizeof(float) * (*nztop) * (*nxtop) * nti, t1xx_py, 0, NULL, NULL);
        CHECK_ERROR(errNum, "InputDataCopyHostToDevice, t1xx_py");
		errNum = clEnqueueWriteBuffer(_cl_commandQueue, t1xy_pyD, CL_TRUE, 0, sizeof(float) * (*nztop) * (*nxtop) * nth, t1xy_py, 0, NULL, NULL);
        CHECK_ERROR(errNum, "InputDataCopyHostToDevice, t1xy_py");
		errNum = clEnqueueWriteBuffer(_cl_commandQueue, t1yy_pyD, CL_TRUE, 0, sizeof(float) * (*nztop) * (*nxtop) * nti, t1yy_py, 0, NULL, NULL);
        CHECK_ERROR(errNum, "InputDataCopyHostToDevice, t1yy_py");
		errNum = clEnqueueWriteBuffer(_cl_commandQueue, t1yz_pyD, CL_TRUE, 0, sizeof(float) * (*nztop+1) * (*nxtop) * nth, t1yz_py, 0, NULL, NULL);
        CHECK_ERROR(errNum, "InputDataCopyHostToDevice, t1yz_py");
		errNum = clEnqueueWriteBuffer(_cl_commandQueue, qt1xx_pyD, CL_TRUE, 0, sizeof(float) * (*nztop) * (*nxtop) * nti, qt1xx_py, 0, NULL, NULL);
        CHECK_ERROR(errNum, "InputDataCopyHostToDevice, qt1xx_py");
		errNum = clEnqueueWriteBuffer(_cl_commandQueue, qt1xy_pyD, CL_TRUE, 0, sizeof(float) * (*nztop) * (*nxtop) * nth, qt1xy_py, 0, NULL, NULL);
        CHECK_ERROR(errNum, "InputDataCopyHostToDevice, qt1xy_py");
		errNum = clEnqueueWriteBuffer(_cl_commandQueue, qt1yy_pyD, CL_TRUE, 0, sizeof(float) * (*nztop) * (*nxtop) * nti, qt1yy_py, 0, NULL, NULL);
        CHECK_ERROR(errNum, "InputDataCopyHostToDevice, qt1yy_py");
		errNum = clEnqueueWriteBuffer(_cl_commandQueue, qt1yz_pyD, CL_TRUE, 0, sizeof(float) * (*nztop+1) * (*nxtop) * nth, qt1yz_py, 0, NULL, NULL);
        CHECK_ERROR(errNum, "InputDataCopyHostToDevice, qt1yz_py");
	}

	errNum = clEnqueueWriteBuffer(_cl_commandQueue, qt1xxD, CL_TRUE, 0, sizeof(float) * (*nztop) * (*nxtop) * (*nytop), qt1xx, 0, NULL, NULL);
    CHECK_ERROR(errNum, "InputDataCopyHostToDevice, qt1xx");
	errNum = clEnqueueWriteBuffer(_cl_commandQueue, qt1xyD, CL_TRUE, 0, sizeof(float) * (*nztop) * (*nxtop) * (*nytop), qt1xy, 0, NULL, NULL);
    CHECK_ERROR(errNum, "InputDataCopyHostToDevice, qt1xy");
	errNum = clEnqueueWriteBuffer(_cl_commandQueue, qt1xzD, CL_TRUE, 0, sizeof(float) * (*nztop+1) * (*nxtop) * (*nytop), qt1xz, 0, NULL, NULL);
    CHECK_ERROR(errNum, "InputDataCopyHostToDevice, qt1xz");
	errNum = clEnqueueWriteBuffer(_cl_commandQueue, qt1yyD, CL_TRUE, 0, sizeof(float) * (*nztop) * (*nxtop) * (*nytop), qt1yy, 0, NULL, NULL);
    CHECK_ERROR(errNum, "InputDataCopyHostToDevice, qt1yy");
	errNum = clEnqueueWriteBuffer(_cl_commandQueue, qt1yzD, CL_TRUE, 0, sizeof(float) * (*nztop+1) * (*nxtop) * (*nytop), qt1yz, 0, NULL, NULL);
    CHECK_ERROR(errNum, "InputDataCopyHostToDevice, qt1yz");
	errNum = clEnqueueWriteBuffer(_cl_commandQueue, qt1zzD, CL_TRUE, 0, sizeof(float) * (*nztop) * (*nxtop) * (*nytop), qt1zz, 0, NULL, NULL);
    CHECK_ERROR(errNum, "InputDataCopyHostToDevice, qt1zz");

	errNum = clEnqueueWriteBuffer(_cl_commandQueue, clamdaD, CL_TRUE, 0, sizeof(float) * (*nmat), clamda, 0, NULL, NULL);
    CHECK_ERROR(errNum, "InputDataCopyHostToDevice, clamda");
	errNum = clEnqueueWriteBuffer(_cl_commandQueue, cmuD, CL_TRUE, 0, sizeof(float) * (*nmat), cmu, 0, NULL, NULL);
    CHECK_ERROR(errNum, "InputDataCopyHostToDevice, cmu");
	errNum = clEnqueueWriteBuffer(_cl_commandQueue, epdtD, CL_TRUE, 0, sizeof(float) * (*nll), epdt, 0, NULL, NULL);
    CHECK_ERROR(errNum, "InputDataCopyHostToDevice, epdt");
	errNum = clEnqueueWriteBuffer(_cl_commandQueue, qwpD, CL_TRUE, 0, sizeof(float) * (*nmat), qwp, 0, NULL, NULL);
    CHECK_ERROR(errNum, "InputDataCopyHostToDevice, qwp");
	errNum = clEnqueueWriteBuffer(_cl_commandQueue, qwsD, CL_TRUE, 0, sizeof(float) * (*nmat), qws, 0, NULL, NULL);
    CHECK_ERROR(errNum, "InputDataCopyHostToDevice, qws");
	errNum = clEnqueueWriteBuffer(_cl_commandQueue, qwt1D, CL_TRUE, 0, sizeof(float) * (*nll), qwt1, 0, NULL, NULL);
    CHECK_ERROR(errNum, "InputDataCopyHostToDevice, qwt1");
	errNum = clEnqueueWriteBuffer(_cl_commandQueue, qwt2D, CL_TRUE, 0, sizeof(float) * (*nll), qwt2, 0, NULL, NULL);
    CHECK_ERROR(errNum, "InputDataCopyHostToDevice, qwt2");

	//for inner_II
	errNum = clEnqueueWriteBuffer(_cl_commandQueue, nd2_txyD, CL_TRUE, 0, sizeof(int) * 18, nd2_txy, 0, NULL, NULL);
    CHECK_ERROR(errNum, "InputDataCopyHostToDevice, nd2_txy");
	errNum = clEnqueueWriteBuffer(_cl_commandQueue, nd2_txzD, CL_TRUE, 0, sizeof(int) * 18, nd2_txz, 0, NULL, NULL); 
    CHECK_ERROR(errNum, "InputDataCopyHostToDevice, nd2_txz");
	errNum = clEnqueueWriteBuffer(_cl_commandQueue, nd2_tyyD, CL_TRUE, 0, sizeof(int) * 18, nd2_tyy, 0, NULL, NULL);
    CHECK_ERROR(errNum, "InputDataCopyHostToDevice, nd2_tyy");
	errNum = clEnqueueWriteBuffer(_cl_commandQueue, nd2_tyzD, CL_TRUE, 0, sizeof(int) * 18, nd2_tyz, 0, NULL, NULL);
    CHECK_ERROR(errNum, "InputDataCopyHostToDevice, nd2_tyz");

	errNum = clEnqueueWriteBuffer(_cl_commandQueue, drti2D, CL_TRUE, 0, sizeof(float) * (*mw2_pml1) * 2, drti2, 0, NULL, NULL);
	CHECK_ERROR(errNum, "InputDataCopyHostToDevice, drti2");
	errNum = clEnqueueWriteBuffer(_cl_commandQueue, drth2D, CL_TRUE, 0, sizeof(float) * (*mw2_pml1) * 2, drth2, 0, NULL, NULL);
	...
	errNum = clEnqueueWriteBuffer(_cl_commandQueue, v2xD, CL_TRUE, 0, sizeof(float) * (*nzbtm + 1) * (*nxbtm + 3) * (*nybtm + 3), v2x, 0, NULL, NULL);
	CHECK_ERROR(errNum, "InputDataCopyHostToDevice, v2x");
	errNum = clEnqueueWriteBuffer(_cl_commandQueue, v2yD, CL_TRUE, 0, sizeof(float) * (*nzbtm + 1) * (*nxbtm + 3) * (*nybtm + 3), v2y, 0, NULL, NULL);
	CHECK_ERROR(errNum, "InputDataCopyHostToDevice, v2y");
	errNum = clEnqueueWriteBuffer(_cl_commandQueue, v2zD, CL_TRUE, 0, sizeof(float) * (*nzbtm + 1) * (*nxbtm + 3) * (*nybtm + 3), v2z, 0, NULL, NULL);
	CHECK_ERROR(errNum, "InputDataCopyHostToDevice, v2z");

	errNum = clEnqueueWriteBuffer(_cl_commandQueue, qt2xxD, CL_TRUE, 0, sizeof(float) * (*nzbtm) * (*nxbtm) * (*nybtm), qt2xx, 0, NULL, NULL);
	CHECK_ERROR(errNum, "InputDataCopyHostToDevice, qt2xx");
	errNum = clEnqueueWriteBuffer(_cl_commandQueue, qt2xyD, CL_TRUE, 0, sizeof(float) * (*nzbtm) * (*nxbtm) * (*nybtm), qt2xy, 0, NULL, NULL);
	CHECK_ERROR(errNum, "InputDataCopyHostToDevice, qt2xy");
	errNum = clEnqueueWriteBuffer(_cl_commandQueue, qt2xzD, CL_TRUE, 0, sizeof(float) * (*nzbtm) * (*nxbtm) * (*nybtm), qt2xz, 0, NULL, NULL);
	CHECK_ERROR(errNum, "InputDataCopyHostToDevice, qt2xz");
	errNum = clEnqueueWriteBuffer(_cl_commandQueue, qt2yyD, CL_TRUE, 0, sizeof(float) * (*nzbtm) * (*nxbtm) * (*nybtm), qt2yy, 0, NULL, NULL);
	CHECK_ERROR(errNum, "InputDataCopyHostToDevice, qt2yy");
	errNum = clEnqueueWriteBuffer(_cl_commandQueue, qt2yzD, CL_TRUE, 0, sizeof(float) * (*nzbtm) * (*nxbtm) * (*nybtm), qt2yz, 0, NULL, NULL);
	CHECK_ERROR(errNum, "InputDataCopyHostToDevice, qt2yz");
	errNum = clEnqueueWriteBuffer(_cl_commandQueue, qt2zzD, CL_TRUE, 0, sizeof(float) * (*nzbtm) * (*nxbtm) * (*nybtm), qt2zz, 0, NULL, NULL);
	...
	if (lbx[1] >= lbx[0])
	{
        nti = (lbx[1] - lbx[0] + 1) * (*mw2_pml) + lbx[1];
        nth = (lbx[1] - lbx[0] + 1) * (*mw2_pml) + 1 - lbx[0];
		errNum = clEnqueueWriteBuffer(_cl_commandQueue, t2xx_pxD, CL_TRUE, 0, sizeof(float) * (*nzbtm) * nti * (*nybtm), t2xx_px, 0, NULL, NULL);
        CHECK_ERROR(errNum, "InputDataCopyHostToDevice, t2xx_px");
		errNum = clEnqueueWriteBuffer(_cl_commandQueue, t2xy_pxD, CL_TRUE, 0, sizeof(float) * (*nzbtm) * nth * (*nybtm), t2xy_px, 0, NULL, NULL);
        CHECK_ERROR(errNum, "InputDataCopyHostToDevice, t2xy_px");
		errNum = clEnqueueWriteBuffer(_cl_commandQueue, t2xz_pxD, CL_TRUE, 0, sizeof(float) * (*nzbtm) * nth * (*nybtm), t2xz_px, 0, NULL, NULL);
        CHECK_ERROR(errNum, "InputDataCopyHostToDevice, t2xz_px");
		errNum = clEnqueueWriteBuffer(_cl_commandQueue, t2yy_pxD, CL_TRUE, 0, sizeof(float) * (*nzbtm) * nti * (*nybtm), t2yy_px, 0, NULL, NULL);
        CHECK_ERROR(errNum, "InputDataCopyHostToDevice, t2yy_px");
		errNum = clEnqueueWriteBuffer(_cl_commandQueue, qt2xx_pxD, CL_TRUE, 0, sizeof(float) * (*nzbtm) * nti * (*nybtm), qt2xx_px, 0, NULL, NULL);
        CHECK_ERROR(errNum, "InputDataCopyHostToDevice, qt2xx_px");
		errNum = clEnqueueWriteBuffer(_cl_commandQueue, qt2xy_pxD, CL_TRUE, 0, sizeof(float) * (*nzbtm) * nth * (*nybtm), qt2xy_px, 0, NULL, NULL);
        CHECK_ERROR(errNum, "InputDataCopyHostToDevice, qt2xy_px");
		errNum = clEnqueueWriteBuffer(_cl_commandQueue, qt2xz_pxD, CL_TRUE, 0, sizeof(float) * (*nzbtm) * nth * (*nybtm), qt2xz_px, 0, NULL, NULL);
        CHECK_ERROR(errNum, "InputDataCopyHostToDevice, qt2xz_px");
		errNum = clEnqueueWriteBuffer(_cl_commandQueue, qt2yy_pxD, CL_TRUE, 0, sizeof(float) * (*nzbtm) * nti * (*nybtm), qt2yy_px, 0, NULL, NULL);
	...
	if (lby[1] >= lby[0])
	{
        nti = (lby[1] - lby[0] + 1) * (*mw2_pml) + lby[1];
        nth = (lby[1] - lby[0] + 1) * (*mw2_pml) + 1 - lby[0];
		errNum = clEnqueueWriteBuffer(_cl_commandQueue, t2xx_pyD, CL_TRUE, 0, sizeof(float) * (*nzbtm) * (*nxbtm) * nti, t2xx_py, 0, NULL, NULL);
        CHECK_ERROR(errNum, "InputDataCopyHostToDevice, t2xx_py");
		errNum = clEnqueueWriteBuffer(_cl_commandQueue, t2xy_pyD, CL_TRUE, 0, sizeof(float) * (*nzbtm) * (*nxbtm) * nth, t2xy_py, 0, NULL, NULL);
        CHECK_ERROR(errNum, "InputDataCopyHostToDevice, t2xy_py");
		errNum = clEnqueueWriteBuffer(_cl_commandQueue, t2yy_pyD, CL_TRUE, 0, sizeof(float) * (*nzbtm) * (*nxbtm) * nti, t2yy_py, 0, NULL, NULL);
        CHECK_ERROR(errNum, "InputDataCopyHostToDevice, t2yy_py");
		errNum = clEnqueueWriteBuffer(_cl_commandQueue, t2yz_pyD, CL_TRUE, 0, sizeof(float) * (*nzbtm) * (*nxbtm) * nth, t2yz_py, 0, NULL, NULL);
        CHECK_ERROR(errNum, "InputDataCopyHostToDevice, t2yz_py");
		errNum = clEnqueueWriteBuffer(_cl_commandQueue, qt2xx_pyD, CL_TRUE, 0, sizeof(float) * (*nzbtm) * (*nxbtm) * nti, qt2xx_py, 0, NULL, NULL);
        CHECK_ERROR(errNum, "InputDataCopyHostToDevice, qt2xx_py");
		errNum = clEnqueueWriteBuffer(_cl_commandQueue, qt2xy_pyD, CL_TRUE, 0, sizeof(float) * (*nzbtm) * (*nxbtm) * nth, qt2xy_py, 0, NULL, NULL);
        CHECK_ERROR(errNum, "InputDataCopyHostToDevice, qt2xy_py");
		errNum = clEnqueueWriteBuffer(_cl_commandQueue, qt2yy_pyD, CL_TRUE, 0, sizeof(float) * (*nzbtm) * (*nxbtm) * nti, qt2yy_py, 0, NULL, NULL);
        CHECK_ERROR(errNum, "InputDataCopyHostToDevice, qt2yy_py");
		errNum = clEnqueueWriteBuffer(_cl_commandQueue, qt2yz_pyD, CL_TRUE, 0, sizeof(float) * (*nzbtm) * (*nxbtm) * nth, qt2yz_py, 0, NULL, NULL);
        CHECK_ERROR(errNum, "InputDataCopyHostToDevice, qt2yz_py");
	}

	errNum = clEnqueueWriteBuffer(_cl_commandQueue, t2xx_pzD, CL_TRUE, 0, sizeof(float) * (*mw2_pml) * (*nxbtm) * (*nybtm), t2xx_pz, 0, NULL, NULL);
    CHECK_ERROR(errNum, "InputDataCopyHostToDevice, t2xx_pz");
	errNum = clEnqueueWriteBuffer(_cl_commandQueue, t2xz_pzD, CL_TRUE, 0, sizeof(float) * (*mw2_pml1) * (*nxbtm) * (*nybtm), t2xz_pz, 0, NULL, NULL);
    CHECK_ERROR(errNum, "InputDataCopyHostToDevice, t2xz_pz");
	errNum = clEnqueueWriteBuffer(_cl_commandQueue, t2yz_pzD, CL_TRUE, 0, sizeof(float) * (*mw2_pml1) * (*nxbtm) * (*nybtm), t2yz_pz, 0, NULL, NULL);
    CHECK_ERROR(errNum, "InputDataCopyHostToDevice, t2yz_pz");
	errNum = clEnqueueWriteBuffer(_cl_commandQueue, t2zz_pzD, CL_TRUE, 0, sizeof(float) * (*mw2_pml) * (*nxbtm) * (*nybtm), t2zz_pz, 0, NULL, NULL);
    CHECK_ERROR(errNum, "InputDataCopyHostToDevice, t2zz_pz");
	errNum = clEnqueueWriteBuffer(_cl_commandQueue, qt2xx_pzD, CL_TRUE, 0, sizeof(float) * (*mw2_pml) * (*nxbtm) * (*nybtm), qt2xx_pz, 0, NULL, NULL);
    CHECK_ERROR(errNum, "InputDataCopyHostToDevice, qt2xx_pz");
	errNum = clEnqueueWriteBuffer(_cl_commandQueue, qt2xz_pzD, CL_TRUE, 0, sizeof(float) * (*mw2_pml1) * (*nxbtm) * (*nybtm), qt2xz_pz, 0, NULL, NULL);
    CHECK_ERROR(errNum, "InputDataCopyHostToDevice, qt2xz_pz");
	errNum = clEnqueueWriteBuffer(_cl_commandQueue, qt2yz_pzD, CL_TRUE, 0, sizeof(float) * (*mw2_pml1) * (*nxbtm) * (*nybtm), qt2yz_pz, 0, NULL, NULL);
    CHECK_ERROR(errNum, "InputDataCopyHostToDevice, qt2yz_pz");
	errNum = clEnqueueWriteBuffer(_cl_commandQueue, qt2zz_pzD, CL_TRUE, 0, sizeof(float) * (*mw2_pml) * (*nxbtm) * (*nybtm), qt2zz_pz, 0, NULL, NULL);
							  ...
							  int	*nzbtm)
{
	...
	//for inner_I
	errNum = clEnqueueReadBuffer(_cl_commandQueue, v1xD, CL_TRUE, 0, sizeof(float) * (*nztop + 2) * (*nxtop + 3) * (*nytop + 3), v1x, 0, NULL, NULL);
	CHECK_ERROR(errNum, "outputDataCopyDeviceToHost1, v1x");
	errNum = clEnqueueReadBuffer(_cl_commandQueue, v1yD, CL_TRUE, 0, sizeof(float) * (*nztop + 2) * (*nxtop + 3) * (*nytop + 3), v1y, 0, NULL, NULL);
	CHECK_ERROR(errNum, "outputDataCopyDeviceToHost1, v1y");
	errNum = clEnqueueReadBuffer(_cl_commandQueue, v1zD, CL_TRUE, 0, sizeof(float) * (*nztop + 2) * (*nxtop + 3) * (*nytop + 3), v1z, 0, NULL, NULL);
	CHECK_ERROR(errNum, "outputDataCopyDeviceToHost1, v1z");

	//for inner_II
	errNum = clEnqueueReadBuffer(_cl_commandQueue, v2xD, CL_TRUE, 0, sizeof(float) * (*nzbtm + 1) * (*nxbtm + 3) * (*nybtm + 3), v2x, 0, NULL, NULL);
	CHECK_ERROR(errNum, "outputDataCopyDeviceToHost1, v2x");
	errNum = clEnqueueReadBuffer(_cl_commandQueue, v2yD, CL_TRUE, 0, sizeof(float) * (*nzbtm + 1) * (*nxbtm + 3) * (*nybtm + 3), v2y, 0, NULL, NULL);
	CHECK_ERROR(errNum, "outputDataCopyDeviceToHost1, v2y");
	errNum = clEnqueueReadBuffer(_cl_commandQueue, v2zD, CL_TRUE, 0, sizeof(float) * (*nzbtm + 1) * (*nxbtm + 3) * (*nybtm + 3), v2z, 0, NULL, NULL);
             ...
             int *nxbtm, int *nybtm, int *nzbtm, int *myid)
{
    ...
    errNum = clEnqueueNDRangeKernel(_cl_commandQueue, _cl_Kernel_velocity_inner_IC, 3, NULL, globalWorkSize, localWorkSize, 0, NULL, NULL);
    if(errNum != CL_SUCCESS)
    {
	...
	if (lbx[1] >= lbx[0])
	{
        ...
        errNum = clEnqueueNDRangeKernel(_cl_commandQueue, _cl_Kernel_vel_PmlX_IC, 3, NULL, globalWorkSize, localWorkSize, 0, NULL, NULL);
        if(errNum != CL_SUCCESS)
        {
	...
	if (lby[1] >= lby[0])
	{
        ...
        clEnqueueNDRangeKernel(_cl_commandQueue, _cl_Kernel_vel_PmlY_IC, 3, NULL, globalWorkSize, localWorkSize, 0, NULL, NULL);
    }
    ...
    errNum = clEnqueueNDRangeKernel(_cl_commandQueue, _cl_Kernel_velocity_inner_IIC, 3, NULL, globalWorkSize, localWorkSize, 0, NULL, NULL);
    if(errNum != CL_SUCCESS)
    {
	...
	if (lbx[1] >= lbx[0])
	{
        ...
        errNum = clEnqueueNDRangeKernel(_cl_commandQueue, _cl_Kernel_vel_PmlX_IIC, 3, NULL, globalWorkSize, localWorkSize, 0, NULL, NULL);
        if(errNum != CL_SUCCESS)
        {
	...
	if (lby[1] >= lby[0])
	{
        ...
        errNum = clEnqueueNDRangeKernel(_cl_commandQueue, _cl_Kernel_vel_PmlY_IIC, 3, NULL, globalWorkSize, localWorkSize, 0, NULL, NULL);
        if(errNum != CL_SUCCESS)
        {
    ...
    errNum = clEnqueueNDRangeKernel(_cl_commandQueue, _cl_Kernel_vel_PmlZ_IIC, 3, NULL, globalWorkSize, localWorkSize, 0, NULL, NULL);
    if(errNum != CL_SUCCESS)
    {
        fprintf(stderr, "Error: queuing kernel _cl_Kernel_vel_PmlZ_IIC for execution!\n");
    }
    
    clFinish(_cl_commandQueue);

	Stop(&kernelTimerVelocity);
    gettimeofday(&t2, NULL);
    tmpTime = 1000.0 * (t2.tv_sec - t1.tv_sec) + (t2.tv_usec - t1.tv_usec) / 1000.0;
						    ...
						    int	  *nzbtm)
{
	...
	errNum = clEnqueueReadBuffer(_cl_commandQueue, t1xxD, CL_FALSE, 0, sizeof(float) * (*nztop) * (*nxtop + 3) * (*nytop), t1xx, 0, NULL, NULL);
	CHECK_ERROR(errNum, "outputDataCopyDeviceToHost, t1xx");
	errNum = clEnqueueReadBuffer(_cl_commandQueue, t1xyD, CL_FALSE, 0, sizeof(float) * (*nztop) * (*nxtop + 3) * (*nytop + 3), t1xy, 0, NULL, NULL);
	CHECK_ERROR(errNum, "outputDataCopyDeviceToHost, t1xy");
	errNum = clEnqueueReadBuffer(_cl_commandQueue, t1xzD, CL_FALSE, 0, sizeof(float) * (*nztop + 1) * (*nxtop + 3) * (*nytop), t1xz, 0, NULL, NULL);
	CHECK_ERROR(errNum, "outputDataCopyDeviceToHost, t1xz");
	errNum = clEnqueueReadBuffer(_cl_commandQueue, t1yyD, CL_FALSE, 0, sizeof(float) * (*nztop) * (*nxtop) * (*nytop + 3), t1yy, 0, NULL, NULL);
	CHECK_ERROR(errNum, "outputDataCopyDeviceToHost, t1yy");
	errNum = clEnqueueReadBuffer(_cl_commandQueue, t1yzD, CL_FALSE, 0, sizeof(float) * (*nztop + 1) * (*nxtop) * (*nytop + 3), t1yz, 0, NULL, NULL);
	CHECK_ERROR(errNum, "outputDataCopyDeviceToHost, t1yz");
	errNum = clEnqueueReadBuffer(_cl_commandQueue, t1zzD, CL_FALSE, 0, sizeof(float) * (*nztop) * (*nxtop) * (*nytop), t1zz, 0, NULL, NULL);
	CHECK_ERROR(errNum, "outputDataCopyDeviceToHost, t1zz");

	errNum = clEnqueueReadBuffer(_cl_commandQueue, t2xxD, CL_FALSE, 0, sizeof(float) * (*nzbtm) * (*nxbtm + 3) * (*nybtm), t2xx, 0, NULL, NULL);
	CHECK_ERROR(errNum, "outputDataCopyDeviceToHost, t2xx");
	errNum = clEnqueueReadBuffer(_cl_commandQueue, t2xyD, CL_FALSE, 0, sizeof(float) * (*nzbtm) * (*nxbtm + 3) * (*nybtm + 3), t2xy, 0, NULL, NULL);
	CHECK_ERROR(errNum, "outputDataCopyDeviceToHost, t2xy");
	errNum = clEnqueueReadBuffer(_cl_commandQueue, t2xzD, CL_FALSE, 0, sizeof(float) * (*nzbtm + 1) * (*nxbtm + 3) * (*nybtm), t2xz, 0, NULL, NULL);
	CHECK_ERROR(errNum, "outputDataCopyDeviceToHost, t2xz");
	errNum = clEnqueueReadBuffer(_cl_commandQueue, t2yyD, CL_FALSE, 0, sizeof(float) * (*nzbtm) * (*nxbtm) * (*nybtm + 3), t2yy, 0, NULL, NULL);
	CHECK_ERROR(errNum, "outputDataCopyDeviceToHost, t2yy");
	errNum = clEnqueueReadBuffer(_cl_commandQueue, t2yzD, CL_FALSE, 0, sizeof(float) * (*nzbtm + 1) * (*nxbtm) * (*nybtm + 3), t2yz, 0, NULL, NULL);
	CHECK_ERROR(errNum, "outputDataCopyDeviceToHost, t2yz");
	errNum = clEnqueueReadBuffer(_cl_commandQueue, t2zzD, CL_FALSE, 0, sizeof(float) * (*nzbtm + 1) * (*nxbtm) * (*nybtm), t2zz, 0, NULL, NULL);
                ...
                void **v2xMp, void **v2yMp, void **v2zMp, int *myid)
{
    ...
    globalWorkSize[1] = dimGrid1[1]*localWorkSize[1];
    globalWorkSize[2] = dimGrid1[2]*localWorkSize[2];
    errNum = clEnqueueNDRangeKernel(_cl_commandQueue, _cl_Kernel_stress_norm_xy_IC, 3, NULL, globalWorkSize, localWorkSize, 0, NULL, NULL);
    if(errNum != CL_SUCCESS)
    {
    ...
    globalWorkSize[1] = dimGrid2[1]*localWorkSize[1];
    globalWorkSize[2] = dimGrid2[2]*localWorkSize[2];
    errNum = clEnqueueNDRangeKernel(_cl_commandQueue, _cl_Kernel_stress_xz_yz_IC, 3, NULL, globalWorkSize, localWorkSize, 0, NULL, NULL);
    if(errNum != CL_SUCCESS)
    {
    ...
    globalWorkSize[1] = dimGrid3[1]*localWorkSize[1];
    globalWorkSize[2] = dimGrid3[2]*localWorkSize[2];
    errNum = clEnqueueNDRangeKernel(_cl_commandQueue, _cl_Kernel_stress_resetVars, 3, NULL, globalWorkSize, localWorkSize, 0, NULL, NULL);
    if(errNum != CL_SUCCESS)
    {
	...
	if (lbx[1] >= lbx[0])
	{
        ...
        globalWorkSize[1] = dimGrid4[1]*localWorkSize[1];
        globalWorkSize[2] = dimGrid4[2]*localWorkSize[2];
        errNum = clEnqueueNDRangeKernel(_cl_commandQueue, _cl_Kernel_stress_norm_PmlX_IC, 3, NULL, globalWorkSize, localWorkSize, 0, NULL, NULL);
        if(errNum != CL_SUCCESS)
        {
	...
	if (lby[1] >= lby[0])
	{
        ...
        globalWorkSize[1] = dimGrid5[1]*localWorkSize[1];
        globalWorkSize[2] = dimGrid5[2]*localWorkSize[2];
        errNum = clEnqueueNDRangeKernel(_cl_commandQueue, _cl_Kernel_stress_norm_PmlY_IC, 3, NULL, globalWorkSize, localWorkSize, 0, NULL, NULL);
        if(errNum != CL_SUCCESS)
        {
	...
	if (lbx[1] >= lbx[0]) 
	{
        ...
        globalWorkSize[1] = dimGrid6[1]*localWorkSize[1];
        globalWorkSize[2] = dimGrid6[2]*localWorkSize[2];
        errNum = clEnqueueNDRangeKernel(_cl_commandQueue, _cl_Kernel_stress_xy_PmlX_IC, 3, NULL, globalWorkSize, localWorkSize, 0, NULL, NULL);
        if(errNum != CL_SUCCESS)
        {
	...
	if (lby[1] >= lby[0])
	{
        ...
        globalWorkSize[1] = dimGrid7[1]*localWorkSize[1];
        globalWorkSize[2] = dimGrid7[2]*localWorkSize[2];
        errNum = clEnqueueNDRangeKernel(_cl_commandQueue, _cl_Kernel_stress_xy_PmlY_IC, 3, NULL, globalWorkSize, localWorkSize, 0, NULL, NULL);
        if(errNum != CL_SUCCESS)
        {
	...
	if (lbx[1] >= lbx[0])
	{
        ...
        errNum = clEnqueueNDRangeKernel(_cl_commandQueue, _cl_Kernel_stress_xz_PmlX_IC, 3, NULL, globalWorkSize, localWorkSize, 0, NULL, NULL);
        if(errNum != CL_SUCCESS)
        {
	...
	if (lby[1] >= lby[0])
	{
        ...
        errNum = clEnqueueNDRangeKernel(_cl_commandQueue, _cl_Kernel_stress_xz_PmlY_IC, 3, NULL, globalWorkSize, localWorkSize, 0, NULL, NULL);
        if(errNum != CL_SUCCESS)
        {
	...
	if (lbx[1] >= lbx[0])
	{
        ...
        errNum = clEnqueueNDRangeKernel(_cl_commandQueue, _cl_Kernel_stress_yz_PmlX_IC, 3, NULL, globalWorkSize, localWorkSize, 0, NULL, NULL);
        if(errNum != CL_SUCCESS)
        {
	...
	if (lby[1] >= lby[0])
	{
        ...
        errNum = clEnqueueNDRangeKernel(_cl_commandQueue, _cl_Kernel_stress_yz_PmlY_IC, 3, NULL, globalWorkSize, localWorkSize, 0, NULL, NULL);
        if(errNum != CL_SUCCESS)
        {
    ...
    errNum = clEnqueueNDRangeKernel(_cl_commandQueue, _cl_Kernel_stress_norm_xy_II, 3, NULL, globalWorkSize, localWorkSize, 0, NULL, NULL);
    if(errNum != CL_SUCCESS)
    {
    ...
    errNum = clEnqueueNDRangeKernel(_cl_commandQueue, _cl_Kernel_stress_xz_yz_IIC, 3, NULL, globalWorkSize, localWorkSize, 0, NULL, NULL);
    if(errNum != CL_SUCCESS)
    {
	...
	if (lbx[1] >= lbx[0])
	{
        ...
        errNum = clEnqueueNDRangeKernel(_cl_commandQueue, _cl_Kernel_stress_norm_PmlX_IIC, 3, NULL, globalWorkSize, localWorkSize, 0, NULL, NULL);
        if(errNum != CL_SUCCESS)
        {
	...
	if (lby[1] >= lby[0])
	{
        ...
        errNum = clEnqueueNDRangeKernel(_cl_commandQueue, _cl_Kernel_stress_norm_PmlY_II, 3, NULL, globalWorkSize, localWorkSize, 0, NULL, NULL);
        if(errNum != CL_SUCCESS)
        {
    ...
    errNum = clEnqueueNDRangeKernel(_cl_commandQueue, _cl_Kernel_stress_norm_PmlZ_IIC, 3, NULL, globalWorkSize, localWorkSize, 0, NULL, NULL);
    if(errNum != CL_SUCCESS)
    {
	...
	if (lbx[1] >= lbx[0])
	{
        ...
        errNum = clEnqueueNDRangeKernel(_cl_commandQueue, _cl_Kernel_stress_xy_PmlX_IIC, 3, NULL, globalWorkSize, localWorkSize, 0, NULL, NULL);
        if(errNum != CL_SUCCESS)
        {
	...
	if (lby[1] >= lby[0])
	{
        ...
        errNum = clEnqueueNDRangeKernel(_cl_commandQueue, _cl_Kernel_stress_xy_PmlY_IIC, 3, NULL, globalWorkSize, localWorkSize, 0, NULL, NULL);
        if(errNum != CL_SUCCESS)
        {
    ...
    errNum = clEnqueueNDRangeKernel(_cl_commandQueue, _cl_Kernel_stress_xy_PmlZ_II, 3, NULL, globalWorkSize, localWorkSize, 0, NULL, NULL);
    if(errNum != CL_SUCCESS)
    {
    ...
    if (lbx[1] >= lbx[0])
	{
        ...
        errNum = clEnqueueNDRangeKernel(_cl_commandQueue, _cl_Kernel_stress_xz_PmlX_IIC, 3, NULL, globalWorkSize, localWorkSize, 0, NULL, NULL);
        if(errNum != CL_SUCCESS)
        {
	...
	if (lby[1] >= lby[0])
	{
        ...
        errNum = clEnqueueNDRangeKernel(_cl_commandQueue, _cl_Kernel_stress_xz_PmlY_IIC, 3, NULL, globalWorkSize, localWorkSize, 0, NULL, NULL);
        if(errNum != CL_SUCCESS)
        {
    ...
    errNum = clEnqueueNDRangeKernel(_cl_commandQueue, _cl_Kernel_stress_xz_PmlZ_IIC, 3, NULL, globalWorkSize, localWorkSize, 0, NULL, NULL);
    if(errNum != CL_SUCCESS)
    {
	...
	if (lbx[1] >= lbx[0])
	{
        ...
        errNum = clEnqueueNDRangeKernel(_cl_commandQueue, _cl_Kernel_stress_yz_PmlX_IIC, 3, NULL, globalWorkSize, localWorkSize, 0, NULL, NULL);
        if(errNum != CL_SUCCESS)
        {
    ...
    if (lby[1] >= lby[0])
    {
        ...
        errNum = clEnqueueNDRangeKernel(_cl_commandQueue, _cl_Kernel_stress_yz_PmlY_IIC, 3, NULL, globalWorkSize, localWorkSize, 0, NULL, NULL);
        if(errNum != CL_SUCCESS)
        {
    ...
    errNum = clEnqueueNDRangeKernel(_cl_commandQueue, _cl_Kernel_stress_yz_PmlZ_IIC, 3, NULL, globalWorkSize, localWorkSize, 0, NULL, NULL);
    if(errNum != CL_SUCCESS)
    {
        fprintf(stderr, "Error: queuing kernel _cl_Kernel_stress_yz_PmlZ_IIC for execution!\n");
    }

    errNum = clFinish(_cl_commandQueue);
    if(errNum != CL_SUCCESS)
    {
        fprintf(stderr, "Error: finishing kernel for execution!\n");
    }

    
    // printf("[OpenCL] computating finished!\n");

	Stop(&kernelTimerStress);
    gettimeofday(&t2, NULL);
    tmpTime = 1000.0 * (t2.tv_sec - t1.tv_sec) + (t2.tv_usec - t1.tv_usec) / 1000.0;
