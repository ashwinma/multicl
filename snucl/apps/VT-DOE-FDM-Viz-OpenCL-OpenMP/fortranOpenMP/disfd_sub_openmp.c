...
void printTimer()
{
    printf("This is OpenMP version\n");
    printf("id = %d, vel, comp =, %.3f\n", procID, totalTimeCompV);
    printf("id = %d, str, comp =, %.3f\n", procID, totalTimeCompS);
//    printf("Total Time = %.3f\n",totalTimeCompV+totalTimeCompS);
}
					  ...
					  float *v1zM)
{
	...
	#pragma omp parallel for private(i,k3,k,dtxz,dtyz,dtzz)
...
// implicit NONE
	...
	#pragma omp parallel for private(ib,lb,kb,i,rth,rti,k,damp0,dmpx2,dmpx1,dmpyz2,dmpyz1,ro1,rox,roy,roz,vtmpx,vtmpy,vtmpz,dtxy,dtyy,dtzy,dtxz,dtyz,dtzz)
				...
				float *v1z_pyM)
{
	...
	for (lb = lby[0]; lb <= lby[1]; lb++)
	{
		...
		for (j = nd1_vel[4*lb]; j <= nd1_vel[1+4*lb]; j++)
		{
			...
			#pragma omp parallel for private(k,damp0,dmpy2,dmpy1,dmpxz2,dmpxz1,ro1,rox,roy,roz,dtxz,dtyz,dtzz,vtmpx,vtmpy,vtmpz)
				 ...
				 float *v2z_pzM)
{
	...
	//for (j = nd2_vel(1); j <= nd2_vel(6); j++)
	#pragma omp parallel for private(i,kb,damp0,k,dmpz2,dmpz1,dmpxy2,dmpxy1,ro1,rox,roy,roz,vtmpx,vtmpy,vtmpz)
	for (j = nd2_vel[0]; j <= nd2_vel[5]; j++)
	{
		//for (i = nd2_vel(7); i <= nd2_vel(12); i++)
		for (i = nd2_vel[6]; i <= nd2_vel[11]; i++)
...
//	printf("int nytop = %d;\n", *nytop);
//	printf("int nztop = %d;\n", *nztop);
//	printf("int mw1_pml = %d;\n", *mw1_pml);
//	printf("int mw1_pml1 = %d;\n", *mw1_pml1);
...
//	printf("int nybtm = %d;\n", *nybtm);
//	printf("int nzbtm = %d;\n", *nzbtm);
//	printf("int mw2_pml = %d;\n", *mw2_pml);
//	printf("int mw2_pml1 = %d;\n", *mw2_pml1);
//	for (i = 0; i < 18; i++)
//	{
//		printf("nd2_txy[%d] = %d;\n", i, nd2_txy[i]);
//	}
...
//        cl,sm2,pm,et,et1,wtp,wts
{
	...
	#pragma omp parallel for private(kodd,i,jkq,k,sxx,syy,sxy,szz,inod,cl,sm2,pm,cusxy,sss,irw,et,et1,wtp,wts,qxx,qyy,qzz,qxy)
...
//                   tfr3=-9./176./ca,  tfr4=1./528./ca
{
	...
	#pragma omp parallel for private(j,kodd,i,jkq,k,dvzx,dvzy,dvxz,dvyz,inod,sm,cusxz,cusyz,irw,et,et1,dmws,qxz,qyz)
	...
	//if you put pragma over head is a bit more. 
...
//        rti,damp2,damp1,cl,sm2,pm,et,et1,wtp,wts
{
	...
	//nti = (lbx[1] - lbx[0] + 1) * mw1_pml + lbx[0]; // --> error nti = 72 here which gives incorrect address
	nti = (lbx[1] - lbx[0] + 1) * mw1_pml + lbx[1];       // nti = 73 which is correct value for nti as per fortran code
	
	#pragma omp parallel for private(kodd,ib,lb,kb,i,rti,jkq,k,damp2,damp1,inod,cl,sm2,pm,irw,et,et1,wtp,wts,taoxx,taoyy,taozz,syy,szz,sss,qxx,qyy,qzz,sxx)
...
//        rti,damp2,damp1,cl,sm2,pm,et,et1,wtp,wts
{
	...
	//do lb=lby(1),lby(2)
	{
		...
		//do j=nd1_tyy(1+4*lb),nd1_tyy(2+4*lb)
		{
			...
			//works OK. Can remove though
			#pragma omp parallel for private(jkq,k,damp2,damp1,inod,cl,sm2,pm,irw,et,et1,wtp,wts,sxx,szz,sss,qyy,qxx,syy,qzz)
...
// real:: taoxy,cusxy,qxy,rth,damp2,damp1,sm,dmws,et,et1
{
	...
	#pragma omp parallel for private(kodd,ib,lb,kb,i,rth,jkq,k,damp2,damp1,inod,sm,irw,et,et1,dmws,qxy,taoxy,cusxy)
...
//real:: taoxy,cusyx,qxy,rth,damp2,damp1,sm,dmws,et,et1
{
	...
	//do lb=lby(1), lby(2)
	{
		...
		//do j=nd1_txy(1+4*lb),nd1_txy(2+4*lb)
		{
			...
			#pragma omp parallel for private(jkq,k,damp2,damp1,inod,sm,irw,et,et1,dmws,taoxy,cusyx,qxy)
...
//real:: taoxz,cusxz,dvxz,qxz,rth,damp2,damp1,sm,dmws,et,et1
{
	...
	#pragma omp parallel for private(kodd,ib,lb,kb,i,rth,jkq,k,damp2,damp1,inod,sm,irw,et,et1,dmws,dvxz,cusxz,qxz,taoxz)
...
//implicit NONE
{
	...
	if (lby[0] > lby[1]) return;
	//if( lby(1)>lby(2) ) return
	// below pragma works but it is slow depends on lb values
	//#pragma omp parallel for private(j,kodd,i,jkq,k,inod,sm,irw,et,et1,dmws,dvzx,cusxz,qxz)
	for (lb=lby[0]; lb <= lby[1]; lb++)
	//do lb=lby(1),lby(2)
	{
		#pragma omp parallel for private(kodd,i,jkq,k,inod,sm,irw,et,et1,dmws,dvzx,cusxz,qxz)
...
//real:: cusyz,dvyz,dvzy,qyz,sm,dmws,et,et1
{
	...
	#pragma omp parallel for private(kodd,lb,i,jkq,k,inod,sm,irw,et,et1,dmws,dvzy,dvyz,cusyz,qyz)
	for (j = nd1_tyz[2]; j <= nd1_tyz[3]; j++)
	//do j=nd1_tyz(3),nd1_tyz(4)
	{
		kodd=2 * ((j+nyb1)&1)+1;
...
//real:: taoyz,cusyz,dvyz,qyz,rth,damp2,damp1,sm,dmws,et,et1
{
	...
	//do lb=lby(1),lby(2)
	{
		...
		//do j=nd1_tyz(1+4*lb),nd1_tyz(2+4*lb)
		{
			...
			#pragma omp parallel for private(jkq,k,damp2,damp1,inod,sm,irw,et,et1,dmws,dvyz,cusyz,qyz,taoyz)
...
//        cl,sm2,et,et1,dmws,pm,wtp,wts
{
	...
	#pragma omp parallel for private(kodd,i,jkq,k,sxx,syy,sxy,szz,sss,inod,cl,sm2,pm,cusxy,irw,et,et1,wtp,wts,qxx,qyy,qzz,qxy)
...
//real:: qxz,qyz,cusxz,cusyz,sm,et,et1,dmws
{
	...
	#pragma omp parallel for private(kodd,i,jkq,k,inod,sm,cusxz,cusyz,irw,et,et1,dmws,qxz,qyz)
...
//       rti,damp2,damp1,cl,sm2,pm,et,et1,wtp,wts
{
	...
	#pragma omp parallel for private(j,kodd,ib,lb,kb,i,rti,jkq,k,damp2,damp1,inod,cl,sm2,pm,irw,et,et1,wtp,wts,taoxx,taoyy,taozz,syy,szz,sss,sxx,qxx,qyy,qzz)
...
//       rti,damp2,damp1,cl,sm2,pm,et,et1,wtp,wts
{
	...
	//do lb=lby(1),lby(2)
	{
		...
		//do j=nd2_tyy(1+4*lb),nd2_tyy(2+4*lb)
		{
			...
			kodd=2*((j+nyb2)&1)+1;
			//kodd=2*mod(j+nyb2,2)+1;
			#pragma omp parallel for private(jkq,k,damp2,damp1,inod,cl,sm2,pm,irw,et,et1,wtp,wts,taoxx,taoyy,taozz,szz,sxx,sss,qxx,qyy,qzz,syy)
...
//       damp2,damp1,cl,sm2,pm,et,et1,wtp,wts
{
	...
	#pragma omp parallel for private(kodd,i,jkq,kb,k,damp2,damp1,inod,cl,sm2,pm,irw,et,et1,wtp,wts,taoxx,taoyy,taozz,sxx,syy,szz,sss,qxx,qyy,qzz)
	for (j = nd2_tyy[0]; j <= nd2_tyy[5]; j++)
	//do j=nd2_tyy(1),nd2_tyy(6)
	{
		kodd=2*((j+nyb2)&1)+1;
...
//real:: taoxy,cusxy,qxy,rth,damp2,damp1,sm,dmws,et,et1
{
	...
	#pragma omp parallel for private(j,kodd,ib,lb,kb,i,rth,jkq,k,damp2,damp1,sm,inod,irw,et,et1,dmws,taoxy,cusxy,qxy)
...
//real:: taoxy,cusxy,qxy,rth,damp2,damp1,sm,dmws,et,et1
{
	...
	//do lb=lby(1),lby(2)
	{
		...
		//do j=nd2_txy(1+4*lb),nd2_txy(2+4*lb)
		{
			...
			kodd=2*((j+nyb2)&1)+1;
			//kodd=2*mod(j+nyb2,2)+1
			#pragma omp parallel for private(jkq,k,damp2,damp1,inod,sm,irw,et,et1,dmws,taoxy,cusxy,qxy)
...
//real:: cusxy,qxy,sm,dmws,et,et1
{
	...
	#pragma omp parallel for private(kodd,i,jkq,k,inod,sm,irw,et,et1,dmws,cusxy,qxy)
...
//real:: taoxz,cusxz,qxz,rth,damp2,damp1,sm,dmws,et,et1
{
	...
	#pragma omp parallel for private(kodd,ib,lb,kb,i,rth,jkq,k,damp2,damp1,inod,sm,irw,et,et1,dmws,taoxz,cusxz,qxz)
...
//real:: dvxz,dvzx,cusxz,qxz,sm,dmws,et,et1
{
	...
	// Below works but slow a bit.
	//#pragma omp parallel for private(lb,j,kodd,i,jkq,k,inod,sm,irw,et,et1,dmws,dvzx,dvxz,cusxz,qxz) 
	for (lb = lby[0]; lb <= lby[1]; lb++)
	//do lb=lby(1),lby(2)
	{
		#pragma omp parallel for private(j,kodd,i,jkq,k,inod,sm,irw,et,et1,dmws,dvzx,dvxz,cusxz,qxz)
...
//real:: taoxz,cusxz,qxz,damp2,damp1,sm,dmws,et,et1
{
	...
	#pragma omp parallel for private(kodd,i,jkq,kb,k,damp2,damp1,inod,sm,irw,et,et1,dmws,taoxz,cusxz,qxz)
	for (j = nd2_txz[0]; j <= nd2_txz[5]; j++)
	//do j=nd2_txz(1),nd2_txz(6)
	{
		kodd = 2*((j+nyb2)&1)+1;
...
//real:: cusyz,qyz,sm,dmws,et,et1
{
	...
	if(lbx[0] > lbx[1]) return;
	//if( lbx(1)>lbx(2) ) return
	#pragma omp parallel for private(kodd,lb,i,jkq,k,inod,irw,et,et1,dmws,cusyz,qyz)
	for (j=nd2_tyz[2]; j <= nd2_tyz[3]; j++)
	//do j=nd2_tyz(3),nd2_tyz(4)
	{
		kodd=2*((j+nyb2)&1)+1;
...
//real:: taoyz,cusyz,qyz,rth,damp2,damp1,sm,dmws,et,et1
{
	...
	//do lb=lby(1),lby(2)
	{
		...
		//do j=nd2_tyz(1+4*lb),nd2_tyz(2+4*lb)
		{
			...
			kodd=2*((j+nyb2)&1)+1;
			//kodd=2*mod(j+nyb2,2)+1
			#pragma omp parallel for private(jkq,damp2,damp1,inod,sm,irw,et,et1,dmws,taoyz,cusyz,qyz,k)
...
//real:: taoyz,cusyz,qyz,damp2,damp1,sm,dmws,et,et1
{
	...
	#pragma omp parallel for private(kodd,jkq,kb,damp2,damp1,inod,sm,irw,et,et1,dmws,qyz,taoyz,cusyz,j,i,k)
