...
float2 pow(float2 x, float2 y) {
	float2 ret;
	ret[0] = snu_pow(x[0], y[0]);
	ret[1] = snu_pow(x[1], y[1]);
	return ret;
}
...
float16 pow(float16 x, float16 y) {
	float16 ret;
	for(int i=0;i<16;i++) {
		ret[i] = snu_pow(x[i], y[i]);
  }
...
double8 pow(double8 x, double8 y) {
	double8 ret;
	for(int i=0;i<8;i++) {
		ret[i] = snu_pow(x[i], y[i]);
  }
...
double16 pow(double16 x, double16 y) {
	double16 ret;
	for(int i=0;i<16;i++) {
		ret[i] = snu_pow(x[i], y[i]);
  }
