...
float atan2(float y, float x){
	return snu_atan2(y, x);
}

float2 atan2(float2 y, float2 x) {
	float2 ret;
	ret[0] = snu_atan2(y[0], x[0]);
	ret[1] = snu_atan2(y[1], x[1]);
	return ret;
}
...
float8 atan2(float8 y, float8 x) {
	float8 ret;
	for(int i=0;i<8;i++) {
		ret[i] = snu_atan2(y[i], x[i]);
  }
...
double8 atan2(double8 y, double8 x) {
	double8 ret;
	for(int i=0;i<8;i++) {
		ret[i] = snu_atan2(y[i], x[i]);
  }
...
double16 atan2(double16 y, double16 x) {
	double16 ret;
	for(int i=0;i<16;i++) {
		ret[i] = snu_atan2(y[i], x[i]);
  }
