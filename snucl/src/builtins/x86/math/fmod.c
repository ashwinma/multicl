...
double snu_fmod(double x, double y);
...
float2 fmod(float2 x, float2 y) {
	float2 ret;
	ret[0] = snu_fmod(x[0], y[0]);
	ret[1] = snu_fmod(x[1], y[1]);
	return ret;
}
...
double8 fmod(double8 x, double8 y) {
	double8 ret;
	for(int i=0;i<8;i++) {
		ret[i] = snu_fmod(x[i], y[i]);
  }
...
double16 fmod(double16 x, double16 y) {
	double16 ret;
	for(int i=0;i<16;i++) {
		ret[i] = snu_fmod(x[i], y[i]);
  }
