...
Program read_dfd
 ...
 double precision, allocatable, dimension(:):: vel,xyz,xyz_all
 double precision, allocatable, dimension(:,:):: vel_tmp
 double precision:: dt,dt_new,rtmp,fc_lp,fc_hp
...
! Interpolation
     ...
     nt_new=idint((npt*dt-dt)/dt_new)+1
...
subroutine output3(tmfile,id_format,nump,dt,velc)
 ...
 double precision, dimension(nump), intent(IN):: velc
 integer:: nerr,myfh,getlen,nlch,j
 integer:: initxdr,ixdrint,ixdrreal8,ixdrrmat,ixdrclose
 double precision:: b0
!
 if(id_format == 1) then
!  SAC
...
!  
!    yo   double precision ARRAY CONTAINING SEQUENCE TO BE INTERPOLATED               
...
!    ni   NUMBER OF SAMPLES of y0 AFTER INTERPOLATION
 ...
 double precision, parameter :: etol=1.e-22
 INTEGER, INTENT(IN) :: n0,ni
 double precision, INTENT(IN) :: tbg,dt0,dti
 double precision, DIMENSION(n0), INTENT(INOUT) :: y0
 double precision, DIMENSION(n0) :: yi,ydif
 INTEGER :: i,j,j0,j1
 double precision :: difu1,difu2,tt,rr,r2,r3,ww1,ww2
