...
#define NBLOCKS 12
    ...
    // 1k through 8M bytes
    int minGroupSize =WAVEFRONT_SIZE;
        ...
        for (opt=OP_MEM_READ ; opt<NUM_MEMORY_OPERATIONS ; ++opt) 
        {
            ...
            for (etype=FLOAT_TYPE ; etype<NUM_ELEM_TYPES ; ++etype)
            {
                ...
                while ((bTests!=0) && (bTests[bIdx].name!=0))
                {
                    ...
                    generateKernel (oss, bTests[bIdx], kName, opt, etype, localMemSize, WAVEFRONT_SIZE, 1024);
                        ...
                        if(globalWorkSize/WAVEFRONT_SIZE < maxWavefronts) {
                            if (verbose)
                                cout << ">> running the " << kName 
                                     ...
                                     << " and number of groups=" << numBlocks << "\n";
                            ...
                            for (int pas=0 ; pas<npasses ; ++pas) 
                            {
                                ...
                                double bdwth = ((double)globalWorkSize*bTests[bIdx].numRepeats*bTests[bIdx].numUnrolls*elemSize)
                                         / double(evKernel.StartEndRuntime());
                                sprintf (sizeStr, "GrpSize:%03d", wsize);

                                std::stringstream tmpstr;
                                tmpstr <<  kName;
                                tmpstr << "-Blocks-";
                                for (int i=numBlocks; i<100; i*=10){
                                    tmpstr << "0";
                                }
                                tmpstr << numBlocks;
                                std::string kNameExt = tmpstr.str();
                                resultDB.AddResult(kNameExt.c_str(), 
                                         sizeStr, "GB/s", bdwth);
