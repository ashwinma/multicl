...
half double2half_rtz(double f)
{
  union { uint64_t u; double f; } fv;
  fv.f = f;

  uint16_t sign     = (fv.u >> 48) & 0x8000;
  uint16_t exponent = (fv.u >> 52) & 0x7FF;
  ...
  // overflow
  if (fv.f >= 0x1.0p16)
    return (half)((fv.f == (FLT_MAX + FLT_MAX))
                  ? (sign|0x7C00) : (sign|0x7BFF));
  ...
  if (fv.f < 0x1.0p-14) {
    fv.f = fv.f * 0x1.0p24;
    return (half)(sign | (uint16_t)fv.f);
  }

  // normal
  fv.f = f;
  fv.u = (fv.u & 0xFFFFFC0000000000ULL) - 0x3F00000000000000ULL;
