...
__global__ void velocity_inner_IC_modified(int	nztop,
					  int 	nztm1,
					  float ca,
					  int	*nd1_vel,
					  float *rhoM,
             ...
             int *nxbtm, int *nybtm, int *nzbtm, int *myid)
{
	...
	cudaEvent_t start_copy, start_comp;
	cudaEvent_t stop_copy, stop_comp;
	float time_copy = 0.0f, time_comp = 0.0f;
	...
	cudaEventElapsedTime(&time_copy, start_copy, stop_copy);
	
	printf("OK!\n");
	printf("Copying data from Host to Device takes %f ms\n", time_copy);
	...
	cudaEventElapsedTime(&time_comp, start_comp, stop_comp);
	
	printf("...............");
	cudaRes = cudaDeviceSynchronize();
	CHECK_ERROR(cudaRes, "Kernel failed");
	printf("OK!\n");
	printf("Computing (myid = %d, grid1 = (%d, %d)) takes %f ms\n", *myid, gridSizeX1, gridSizeY1, time_comp);
	...
	if (lbx[1] >= lbx[0])
	{
		...
		CHECK_ERROR(cudaRes, "Kernel failed");
		printf("OK!\n");
		printf("Computing (myid = %d, grid1 = (%d, %d)) takes %f ms\n", *myid, gridSizeX2, gridSizeY2, time_comp);
	}
...
*/
  ...
  totalTimeD2HV += tmpTime;
	...
	cudaEventDestroy(stop_comp);
	cudaEventDestroy(start_comp);
  printf("--------------------GPU-Ends--------------------------\n");
  return;
}
