...
int read_lammps_data(Atom &atom, Comm &comm, Neighbor &neighbor, Integrate &integrate, Thermo &thermo, char* file, int units)
{
  ...
  MMD_float3* x = new MMD_float3[atom.natoms];
  MMD_float3* v = new MMD_float3[atom.natoms];
  ...
  for(int i = 0; i < atom.natoms; i++) {
    if(x[i].x >= atom.box.xlo && x[i].x < atom.box.xhi &&
        x[i].y >= atom.box.ylo && x[i].y < atom.box.yhi &&
        x[i].z >= atom.box.zlo && x[i].z < atom.box.zhi)
      atom.addatom(x[i].x, x[i].y, x[i].z, v[i].x, v[i].y, v[i].z);
  }
