...
float2 mix(float2 x, float2 y, float2 a){
	float2 ret;
	ret.v[0] = x.v[0]+(y.v[0]-x.v[0])*a.v[0];
	ret.v[1] = x.v[1]+(y.v[1]-x.v[1])*a.v[1];
	return ret;
}
...
float3 mix(float3 x, float3 y, float3 a){
	float3 ret;
  for (int i=0; i<3; ++i) {
    ret.v[i] = x.v[i]+(y.v[i]-x.v[i])*a.v[i];
  }
	return ret;
}
...
float2 mix(float2 x, float2 y, float a){
	float2 ret;
	ret.v[0] = x.v[0]+(y.v[0]-x.v[0])*a;
	ret.v[1] = x.v[1]+(y.v[1]-x.v[1])*a;
	return ret;
}
