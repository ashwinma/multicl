module fd_model_comm
 ...
 double precision, parameter:: ar_ha=-1.0, errm=0.001
! vm4fd 
 integer:: ntype,mtype
 integer, allocatable, dimension(:):: idmat
 double precision, allocatable, dimension(:):: evp,evs,erh,eqp,eqs
! viscty
 double precision, dimension(nll):: epdt,qwt1,qwt2,relaxt
! 2D interpolation 
 integer, allocatable, dimension(:,:,:):: idfxy
 double precision, allocatable, dimension(:,:,:):: cefxy,rbkly,rmuly, & 
                                       rholy,qply,qsly
! inorder
...
! bottom
 ...
 double precision, allocatable, dimension(:):: dxx0,dyy0,dzz0
! fdnode
 integer :: nx_fd,ny_fd,nz_fd
 double precision, allocatable, dimension(:):: dxx,dyy,dzz
...
module vmodel_inp
 ...
 double precision:: dx_vm,dy_vm,dz_vm
 integer, allocatable, dimension(:,:):: nodexyz
 double precision, allocatable, dimension(:,:):: gdsp_vm,rbkmd,rmumd,roth,qpmd,qsmd
 double precision, allocatable, dimension(:):: top_depth
 double precision:: vpmin,vsmin,rohmin,qpmin,qsmin,qqmax
...
!------------------------------------------------------------------------|
 ...
 double precision:: depth_I_II,fmax,freq_ref,anglex,angley,xmd0,ymd0
 double precision:: tstep,gstep,ztb1,ztb2,z2_top
 double precision:: depth_bm,depth_up,z0,zz,dzbm,zr2,dt
 double precision:: vpmx1,vpmn1,vsmx1,vsmn1,rhmx1,rhmn1,qpmx1,qpmn1,qsmx1,qsmn1
 double precision, allocatable, dimension(:):: dxseg,distx,dyseg,disty,dzseg,distz
!
...
subroutine set_vm_dim(ncx,ncy,ncz,anglex,angley,xmd0,ymd0, &
 ...
 double precision, intent(INOUT):: anglex,angley,xmd0,ymd0
 double precision, intent(IN):: distx(ncx),disty(ncy),distz(ncz)
 integer:: nof,kup,k,modlx,modly,ntmp(3)
 double precision:: z0, zz, rtmp(3) 
!
...
subroutine readvel(myfh,nfile,kzsub_vm,depth_bm)
 ...
 double precision, intent(INOUT):: depth_bm
 integer:: id_vm,ix,jy
 double precision:: xxp,yyp,depth,thick,vpmd,vsmd,den,qpm,qsm,cmu
...
subroutine inter_face(fmax,dzbtm)
 ...
 double precision, intent(OUT):: dzbtm
 integer:: ix,jy,kz,nfile,nzz
 double precision:: dx,dx1,vs1,vs2,vsmd
 double precision, dimension(modlz):: vhm,hhh
!
...
subroutine h2d_interp(kbm,nx_fd,ny_fd)
 ...
 double precision:: rrx,rry,rr1,rr2,rr3,rr4
...
subroutine smooth(nfile,kz_sub,kbm,nx_fd,ny_fd,ztb1,ztb2)
 ...
 double precision, intent(IN):: ztb1,ztb2
 integer, save:: nlsm,nsmf(4),nsmkz(4)
 integer:: i,j,k,kk,ix1,jy1,ix2,jy2,nkz,nck,icheck
 double precision::  zz1,zz2,sm1,sm2
 double precision, dimension(nx_fd, ny_fd):: vmtp
!
...
subroutine v1d_interp(zr2,k,kup,kbm,tstep,gstep)
 ...
 double precision, intent(IN):: zr2
 double precision, intent(INOUT):: tstep,gstep
 integer:: ntp1,ntp2,i,j,ii,lls,iselc
 double precision:: zr1,ro,cbk,cmu,qp,qs,vp,vs,delt
...
subroutine search(vvs,ntp1,ntp2)
 ...
 double precision, intent(IN):: vvs
 integer:: nn1,nn2,nnn
 double precision:: vv1,vv2,vs0
!
...
subroutine switch(ks)
 ...
 integer:: i,j,k,i3,j3
 double precision:: dx,vs1,vs2,vsmd,depth_bm
...
subroutine coef_interp(anglex,angley,xmd0,ymd0)
 ...
 use fd_model_comm
 implicit NONE
 double precision, intent(IN):: anglex,angley,xmd0,ymd0
 integer:: i,j,ixm,jym 
 double precision:: dc,crx,snx,cry,sny,xf,dxf,yf,dyf,xm,ym,xr,yr
 dc=atan(1.)/45.0
...
subroutine grid_space(id_xyz,nseg,xx,dx)
 ...
 double precision, dimension(nseg), intent(IN):: dx
 double precision, dimension(nseg), intent(INOUT):: xx
 double precision, allocatable, dimension(:):: gsp
 integer:: i,ks,nnode
 double precision:: dseg,slop,af1,afa,sss,x0,xi
!
 Type:: xyz_list
...
subroutine format_dt(dt)
 ...
 integer:: nn
 double precision:: dtsetp,dd,dti
!
 dtsetp=dt
 if(dtsetp > 1.0 ) then
   nn=int(dtsetp*100.0)
...
subroutine smq_coef(id_smq,dt,freq_ref,qsmin,qsmax,qpmin,qpmax)
 ...
 double precision, intent(IN):: dt,freq_ref
 double precision, intent(OUT):: qsmin,qsmax,qpmin,qpmax
 integer:: i,j,nm
 double precision, dimension(nll):: qwt,rt_tmp,wt1_tmp,wt2_tmp
 double precision:: unrelaxed_mu,real_mu,cmur,qq,deltm
!
...
subroutine relx_wt_coef(id_smq,qq,deltm,relaxt,qwt1,qwt2,qwt)
 ...
 double precision, intent(OUT):: deltm
 double precision, dimension(8), intent(OUT):: relaxt,qwt1,qwt2,qwt
!
 select case(id_smq)
 case (1)
...
! f> 3Hz Qs like Qo*(f/3)**(-0.7)
 ...
 double precision, parameter:: Qmin=5, Qmax=5000
 double precision, intent(IN):: qq
 double precision, intent(OUT):: deltm
 double precision, dimension(np), intent(out):: relaxt,qwt1,qwt2,qwt
 double precision :: qr, a1=6.348, a2=-2.166, a3=-0.724, a4=0.094
 ...
 qr=dlog(qq/Qmin);  deltm=(a1+a2*qr*qq**a3)/(1.0+a4*qq)
...
subroutine rw_ha_10hz2(qqi,deltm,relaxt,qwt1,qwt2,qwt)
 ...
 integer, parameter:: np=8
 double precision, parameter:: Qmin=5, Qmax=5000
 double precision, intent(IN):: qqi
 double precision, intent(OUT):: deltm
 double precision, dimension(np), intent(out):: relaxt,qwt1,qwt2,qwt
 double precision:: q1,q2,qq, a1=1.0, a2=-0.0044, a3=0.788927, a4=0.018050
 ...
 q1=dlog(qq/Qmin);  q2=dlog(Qmax/qq)
...
! f=0-10 Hz, Qs are constant
 ...
 double precision, intent(OUT):: deltm
 double precision, dimension(np), intent(out):: relaxt,qwt1,qwt2,qwt
 double precision:: qr, a1= 5.649, a2=-0.477, a3=-0.530, a4= 0.109
...
!
 qr=dlog(amin1(amax1(qq,Qmin), Qmax)/Qmin)
...
! ar_ha=1.0, Arithmetic Average of Modulus
 ...
 double precision, intent(IN):: fr,rmu_vm,ar_ha 
 double precision, dimension(nparam), intent(IN):: relaxt,qwt
 integer:: i
 complex:: cxlnf
 double precision:: unrelaxed_mu,ww,rmus,rtmp
 ww=fr*atan(1.0)*8.0
 cxlnf=(0.0, 0.0)
 rmus=0.0
 if(ar_ha > 0.5) then
   ...
   do i=1,nparam
     ...
     rmus=rmus+cabs(real(1.0-qwt(i))/cmplx(1.,relaxt(i)*ww))
   enddo
   cxlnf=nparam/cxlnf
 endif
 rtmp=cabs(cxlnf)
 unrelaxed_mu=rmu_vm/rtmp*(1.+dble(cxlnf)/rtmp)/2.
...
function cmu_freq(cmu_ref,ww,ar_ha,nparam,relaxt,qwt)
 ...
 integer, intent(IN):: nparam
 double precision, intent(IN):: cmu_ref,ww,ar_ha 
 double precision, dimension(nparam), intent(IN):: relaxt,qwt
 integer:: i
...
! ar_ha=1.0, Arithmetic Average of Modulus
 implicit NONE
 integer, intent(IN):: nparam
 double precision, intent(IN):: ww,ar_ha 
 double precision, dimension(nparam), intent(IN):: relaxt,qwt
 integer:: i
 double precision:: smq,sum1,sum2,wti,wt2,aa,bb,ab2
 sum1=0.0
 sum2=0.0
 do i=1,nparam
   wti=ww*relaxt(i)
