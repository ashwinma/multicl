...
! Input all the parameters need for calculation
 ...
 double precision:: dxsub,dysub,cxp,cyp
 double precision, dimension(8):: rltemp
...
!
 ...
 ncount=8
 call MPI_BCAST (rltemp, ncount,MPI_REAL8,0,MPI_COMM_WORLD,ierr)
 intprt        = intemp(1)
 nproc_x       = intemp(2)
 nproc_y       = intemp(3)
...
!
 ...
 double precision, intent(IN):: RF0
...
!
 ncount=node_all(1)
 call MPI_BCAST (dx_all,ncount, MPI_REAL8, 0,comm_worker,ierr)
 ncount=node_all(2)
 call MPI_BCAST (dy_all,ncount, MPI_REAL8, 0,comm_worker,ierr)
 ncount=node_all(3)+node_all(4)
 call MPI_BCAST (dz_all,ncount, MPI_REAL8, 0,comm_worker,ierr)
!
...
!
 ...
 call MPI_BCAST (dt,   1, MPI_REAL8,    0, comm_worker, ierr)
!
 deallocate(node_all)
...
! write(*,*) 'after allocate(clamda(nmat),cmu(nmat),qwp(nmat),qws(nmat))'
 real_8_itemsize =4
 call MPI_Type_extent(MPI_REAL8, real_8_itemsize, ierror)
 size=nmat*real_8_itemsize
...
!
 ncount=nll
 call MPI_BCAST (epdt, ncount, MPI_REAL8, 0, comm_worker, ierr)
 call MPI_BCAST (qwt1, ncount, MPI_REAL8, 0, comm_worker, ierr)
 call MPI_BCAST (qwt2, ncount, MPI_REAL8, 0, comm_worker, ierr)
...
subroutine read_mvl(mmat,nmat,comm_worker,myid,idmat_tmp,cmat)
 ...
 double precision, dimension(nmat), intent(OUT):: cmat 
 double precision, dimension(mmat):: cmat_tmp
 integer:: k,k2,ierr

 if(myid==0)  read(15) cmat_tmp
!
 call MPI_BCAST (cmat_tmp, mmat, MPI_REAL8, 0, comm_worker, ierr)
 do k=1,mmat
   k2=idmat_tmp(k)
   if(k2 == 0) cycle
   cmat(k2)=cmat_tmp(k)
...
! use itface_comm
 ...
 double precision, intent(IN):: xref_fdm,yref_fdm,angle_north_to_x
 character(len=*), intent(IN):: fileRec
 character(len=72):: stat_name
 integer:: j,k,ncount,ierr
 double precision:: dr,xcr,ycr,zcr,afa_stn,xref_stn,yref_stn,csaf,snaf
 character(len=72), allocatable, dimension(:):: rname_tmp
 double precision, allocatable, dimension(:,:):: recev_tmp
!
 if(myid == 0) then
...
! Input x, y, z (coordinates of receiver, z positive downward) 
 ...
 ncount=nr_all*4
 call MPI_BCAST (recev_tmp,ncount,MPI_REAL8,0,comm_worker,ierr)
!
 nrecs=0
 do j=1,nr_all
...
subroutine Assign_Rev_2_Proc(nrcv,recev_tmp)
 ...
 double precision, dimension(4,nrcv), intent(INOUT):: recev_tmp
...
subroutine Type2_Recv_Int(nrcv,recev_tmp)
 ...
 implicit NONE
 integer, intent(IN):: nrcv
 double precision, dimension(9,nrcv), intent(INOUT):: recev_tmp
 integer:: kc,i1,i2,i3,ni,nx1b,ny1b
 double precision:: xs,dx1,dy1,dx2,dy2,dz1,dz2,zbtm0
!
 nx1b=nx2_all*3-2
 ny1b=ny2_all*3-2
...
subroutine Type2_Rev_Location(nrc3,recv_xyz)
 ...
 double precision, dimension(nrc3), intent(OUT):: recv_xyz
...
! read and set the source parameters of double couple force
 ...
 double precision, intent(IN):: dt,xref_fdm,yref_fdm,angle_north_to_x, &
 ...
 double precision:: xb1,xb2,yb1,yb2,xs,ys,csaf,snaf
 double precision::  pi
 double precision:: sourc_tmp(10,maxSou), sp(10)
...
!^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
   ncount=10*ncount   
   call MPI_BCAST (sourc_tmp,ncount,MPI_REAL8,0, comm_worker, ierr)
   ncount=ncount/10   
   j=1
   do j=1,ncount
     sp=sourc_tmp(:,j)
...
! id_pvh = 1, P-wave; =2, SV-wave; =3, SH-wave
 ...
 if(is_moment /= 9) then
   if(nfadd == 0) then
     deallocate(index_xyz_source, famp, ruptm, riset, sparam2)
     write(*,*) 'deallocated index_xyz_source, famp, ruptm, riset, sparam2'
   else
     allocate(itmp(3,nfadd,4), rtmp(nfadd,9))
     itmp=index_xyz_source(1:3,1:nfadd,1:4)
     rtmp(:,1:6)= famp(1:nfadd,1:6)
...
! set the source parameters of double couple force
 ...
 double precision, dimension(10), intent(IN):: sp
 integer:: ix,jy,kz,ii,ijk,kzb,isu,nsou
 integer, dimension(72,4):: ix2,jy2,kz2
 double precision :: arsp,cof,rmoment,volume
 double precision:: st,dp,rk,csst,snst,csdp,sndp,csrk,snrk
 double precision:: vslip(3),vnorm(3),tensor(6),cf8(72,4)
!
 arsp=sp(4)
...
subroutine normalize(ii,ierr)
 ...
 integer, intent(OUT):: ierr
 double precision, parameter:: pi=3.1415926, pi2=2.*pi
 double precision, parameter:: cft4=0.20,c41=0.60,c42=1.-c41
 double precision, parameter:: cft5=0.20,c51=0.50,c52=1.-c51
 integer:: k
 double precision:: tt1,tt2,coef_norm,gammln
!
 coef_norm=1.0
...
subroutine initial(myid,is_moment,dt)
 ...
 double precision, intent(IN):: dt
 integer:: i,j,k,iseed
! double precision:: pertV=1.e-28, pertS=1.e-25, pertQ=1.e-26
 double precision:: pertV=0.0, pertS=0.0, pertQ=0.0
!
 call coef_interp
 iseed=3
! Region I
...
! use remapping_interface
 ...
 integer(kind=MPI_ADDRESS_KIND) size 
 integer (8):: real_8_itemsize, integer_itemsize
 integer :: ierror
!
 order_imsr=-1
 pid_ims   =-1
...
! allocate(dyi1(4,nytop), dzh1(4,nztop+1), dzi1(4,nztop+1) )
 ...
 size=4*(nztop+1)*real_8_itemsize
...
!   allocate(damp1_x(nzt1,nytop,i:j),damp2_x(nzbtm,nybtm,i:j))
   size=nzt1*nytop*(j-i+1)*real_8_itemsize
   ...
   size=nzbtm*nybtm*(j-i+1)*real_8_itemsize
...
!   allocate(t1xy_px(nztop,nth,nytop), qt1xy_px(nztop,nth,nytop))
   size=nztop*nv2*nytop*real_8_itemsize
   ...
   size=nztop*nti*nytop*real_8_itemsize
   ...
   size=nzt1*nth*nytop*real_8_itemsize
   ...
   size=nztop*nth*nytop*real_8_itemsize
...
!   allocate(t2xy_px(nzbtm,nth,nybtm), qt2xy_px(nzbtm,nth,nybtm))
   size=nzbtm*nv2*nybtm*real_8_itemsize
   ...
   size=nzbtm*nti*nybtm*real_8_itemsize
   ...
   size=nzbtm*nth*nybtm*real_8_itemsize
...
!   allocate(abcx1(nx6:ny6,nztop,nytop), abcx2(nx6:ny6,nzbtm,nybtm))
   size=(ny6-nx6+1)*nztop*nytop*real_8_itemsize
   ...
   size=(ny6-nx6+1)*nzbtm*nybtm*real_8_itemsize
...
!   allocate(damp1_y(nzt1,nxtop,i:j),damp2_y(nzbtm,nxbtm,i:j))
   size=nzt1*nxtop*(j-i+1)*real_8_itemsize
   ...
   size=nzbtm*nxbtm*(j-i+1)*real_8_itemsize
...
!   allocate(t1xy_py(nztop,nxtop,nth), qt1xy_py(nztop,nxtop,nth))
   size=nztop*nv2*nxtop*real_8_itemsize
   ...
   size=nztop*nxtop*nti*real_8_itemsize
   ...
   size=nzt1*nth*nxtop*real_8_itemsize
   ...
   size=nztop*nth*nxtop*real_8_itemsize
...
!   allocate(t2xy_py(nzbtm,nxbtm,nth), qt2xy_py(nzbtm,nxbtm,nth))
   size=nzbtm*nv2*nxbtm*real_8_itemsize
   ...
   size=nzbtm*nti*nxbtm*real_8_itemsize
   ...
   size=nzbtm*nth*nxbtm*real_8_itemsize
...
!   allocate(abcy1(nx6:ny6,nztop,nxtop), abcy2(nx6:ny6,nzbtm,nxbtm))
   size=(ny6-nx6+1)*nztop*nxtop*real_8_itemsize
   ...
   size=(ny6-nx6+1)*nzbtm*nxbtm*real_8_itemsize
...
! allocate(drvh1(mw1_pml1,0:1), drvh2(mw2_pml1,0:1) )
 size=mw1_pml1*2*real_8_itemsize
 ...
 size=mw2_pml1*2*real_8_itemsize
...
! allocate(damp2_z(nxbtm,nybtm))
 size=nxbtm*nybtm*real_8_itemsize
...
! allocate(v2z_pz(mw2_pml,nxbtm,nybtm))
 size=mw2_pml*nxbtm*nybtm*real_8_itemsize
...
! allocate(abcz(4,nxbtm,nybtm))
 ...
 size=mw2_pml1*nxbtm*nybtm*real_8_itemsize
 ...
 size=4*nxbtm*nybtm*real_8_itemsize
...
! allocate(v1z(0:nztop+1, 0:nxtop+2, 0:nytop+2))
 ...
 size=(nztop+2)*(nxtop+3)*(nytop+3)*real_8_itemsize
...
! allocate(t1zz(nztop,     nxtop,     nytop))
 size=nztop*(nxtop+3)*nytop*real_8_itemsize
 ...
 size=nztop*(nxtop+3)*(nytop+3)*real_8_itemsize
 ...
 size=(nztop+1)*(nxtop+3)*nytop*real_8_itemsize
 ...
 size=nztop*nxtop*(nytop+3)*real_8_itemsize
 ...
 size=(nztop+1)*nxtop*(nytop+3)*real_8_itemsize
 ...
 size=nztop*nxtop*nytop*real_8_itemsize
...
! allocate(qt1xz(nztop+1,nxtop,nytop), qt1yz(nztop+1,nxtop,nytop))
 size=(nztop+1)*nxtop*nytop*real_8_itemsize
...
! allocate(v2z(0:nzbtm, 0:nxbtm+2, 0:nybtm+2))
 size=(nzbtm+1)*(nxbtm+3)*(nybtm+3)*real_8_itemsize
...
! allocate(t2zz(0:nzbtm,   nxbtm,     nybtm))
 size=nzbtm*(nxbtm+3)*nybtm*real_8_itemsize
 ...
 size=nzbtm*(nxbtm+3)*(nybtm+3)*real_8_itemsize
 ...
 size=(nzbtm+1)*(nxbtm+3)*nybtm*real_8_itemsize
 ...
 size=nzbtm*nxbtm*(nybtm+3)*real_8_itemsize
 ...
 size=(nzbtm+1)*nxbtm*(nybtm+3)*real_8_itemsize
 ...
 size=(nzbtm+1)*nxbtm*nybtm*real_8_itemsize
...
!allocate(qt2yz(nzbtm,nxbtm,nybtm), qt2zz(nzbtm,nxbtm,nybtm))
 size=nzbtm*nxbtm*nybtm*real_8_itemsize
...
! allocate(rcy52(nzbtm,nxbtm,5),rcy42(nzbtm,nxbtm,4))
 size=nztop*nytop*5*real_8_itemsize
 ...
 size=nztop*nxtop*5*real_8_itemsize
 ...
 size=nzbtm*nybtm*5*real_8_itemsize
 ...
 size=nzbtm*nxbtm*5*real_8_itemsize
 ...
 size=nztop*nytop*4*real_8_itemsize
 ...
 size=nztop*nxtop*4*real_8_itemsize
 ...
 size=nzbtm*nybtm*4*real_8_itemsize
 ...
 size=nzbtm*nxbtm*4*real_8_itemsize
...
subroutine Pml_coef(myid,dt)
 ...
 double precision:: vp,vs,gridsp,damp0
 double precision, dimension(4):: cfabc
!
! PML-X-I
 b1=lbx(1)
 b2=lbx(2)
 if(b2 >= b1) then
   do i=b1,b2
     ...
     damp0=0.5*dt*dlog(1./RF)*(npow+1.)/2./(mw2_pml*gridsp)
     do j=1,nytop
     do k=1,nztop+1
...
! Interpolating velocity on the interface betwee Region I and II
 ...
 call ISEND_IRECV_Velocity(myid, comm_worker)
 call record_time(tend)
 call log_timing("ISEND_IRECV_Velocity", tend-tstart)

 call record_time(tstart)
 call velocity_interp(comm_worker)
 call record_time(tend)
 call log_timing("velocity_interp:", tend-tstart)
 
 call record_time(tstart)
 call vxy_image_layer(comm_worker)
 call record_time(tend)
 call log_timing("vxy_image_layer:", tend-tstart)
 return
...
!     end subroutine vxy_image_layer_vel1
     ...
     subroutine vxy_image_layer_vel1 ( nd1_vel, i, dzdx, &
       ...
       bind(C, name="vxy_image_layer_vel1")
        ...
        real(kind=c_double), intent(in), value:: dzdx
 ...
 double precision:: dzdt,dzdx,dzdy
 double precision, dimension(5):: sendv,recev
 integer::sizeof_float
 double precision:: time_marsh=0.0
...
! finding X- and Y-componet velocity on the image layer.
 ...
 call log_string("----------SUBROUTINE vxy_image_layer")
#endif
 total_gpu_marsh_time =0.0
 total_cpu_marsh_time =0.0

! if(DETAILED_TIMING .eq. 1) then;  call record_time(gpu_tstart) ; end if
! call vxy_image_layer_vel1(cptr_v1x, cptr_v1y, cptr_v1z, cptr_v2x, cptr_v2y, cptr_v2z, &
!                        neighb(1), neighb(2), neighb(3), neighb(4), &
!                        nxtm1, nytm1, nxtop, nytop, nztop,  nxbtm, nybtm, nzbtm )
! if(DETAILED_TIMING .eq. 1) then; call record_time(gpu_tend); call log_timing ("GPU vxy_image_layer_vel1", gpu_tend-gpu_tstart); end if


! if(DETAILED_TIMING .eq. 1) then;  call record_time(cpu_tstart) ; end if                             
 dzdt=ca/dzh1(3,1)
!--Vx
 iix=nd1_vel(12)
 ...
 if(DETAILED_TIMING .eq. 1) then;  call record_time(cpu_tstart) ; end if                             
   ...
   enddo
 if(DETAILED_TIMING .eq. 1) then; call record_time(cpu_tend); call log_timing ("CPU vel1_vxy_image_layer", cpu_tend-cpu_tstart);  &
        total_cpu_marsh_time = total_cpu_marsh_time + (cpu_tend-cpu_tstart); end if
 call validate(v1x_meta)
#endif
 endif
 ...
 enddo
 enddo
 if(DETAILED_TIMING .eq. 1) then; call record_time(cpu_tend); call log_timing ("CPU vel2_vxy_image_layer", cpu_tend-cpu_tstart);  &
        total_cpu_marsh_time = total_cpu_marsh_time + (cpu_tend-cpu_tstart); end if
 call validate(v1x_meta)
#endif 
!!---Vy
 ...
 if(DETAILED_TIMING .eq. 1) then;  call record_time(cpu_tstart) ; end if                             
   ...
   enddo
 if(DETAILED_TIMING .eq. 1) then; call record_time(cpu_tend); call log_timing ("CPU vel3_vxy_image_layer", cpu_tend-cpu_tstart);  &
        total_cpu_marsh_time = total_cpu_marsh_time + (cpu_tend-cpu_tstart); end if
 call validate(v1y_meta)
#endif   
 endif
 ...
 enddo
 enddo
 if(DETAILED_TIMING .eq. 1) then; call record_time(cpu_tend); call log_timing ("CPU vxy_image_layer_vel4", cpu_tend-cpu_tstart);  &
        total_cpu_marsh_time = total_cpu_marsh_time + (cpu_tend-cpu_tstart); end if
 call validate(v1y_meta)
#endif
...
select case(order_imsr) 
 case(1)
#if USE_MPIX == 0 || VALIDATE_MODE ==1
   call MPI_RECV(v1x(0,nx1p1,ny1p1),nnum, MPI_REAL8, &
                 pid_imr,21,comm_worker,status,ierr)
   call MPI_RECV(v1y(0,nx1p1,ny1p1),nnum, MPI_REAL8, &
                 pid_imr,22,comm_worker,status,ierr)
   call MPI_RECV(v1z(0,nx1p1,ny1p1),nnum, MPI_REAL8, &
                 pid_imr,23,comm_worker,status,ierr)
   call MPI_RECV(v2x(0,nx2p1,ny2p1),nzbtm, MPI_REAL8, &
                 pid_imr,24,comm_worker,status,ierr)
   call MPI_RECV(v2y(0,nx2p1,ny2p1),nzbtm, MPI_REAL8, &
                 pid_imr,25,comm_worker,status,ierr)
   call MPI_RECV(v2z(0,nx2p1,ny2p1),nzbtm, MPI_REAL8, &
                pid_imr,26,comm_worker,status,ierr)
#endif               

#if USE_MPIX == 0 && VALIDATE_MODE == 1 
   call upload_to_gpu(cptr_v1x, v1x_meta)
   call upload_to_gpu(cptr_v1y, v1y_meta)
   call upload_to_gpu(cptr_v1z, v1z_meta)
   call upload_to_gpu(cptr_v2x, v2x_meta)
   call upload_to_gpu(cptr_v2y, v2x_meta)
   call upload_to_gpu(cptr_v2z, v2x_meta)
#endif 

#if USE_MPIX == 1
   call MPIX_RECV_offset_C(v1xD(0,nx1p1,ny1p1),nnum, MPI_REAL8, &
                 pid_imr,211,comm_worker,status(1),ierr)
   call MPIX_RECV_offset_C(v1yD(0,nx1p1,ny1p1),nnum, MPI_REAL8, &
                 pid_imr,221,comm_worker,status(1),ierr)
   call MPIX_RECV_offset_C(v1zD(0,nx1p1,ny1p1),nnum, MPI_REAL8, &
                 pid_imr,231,comm_worker,status(1),ierr)
   call MPIX_RECV_offset_C(v2xD(0,nx2p1,ny2p1),nzbtm, MPI_REAL8, &
                 pid_imr,241,comm_worker,status(1),ierr)
   call MPIX_RECV_offset_C(v2yD(0,nx2p1,ny2p1),nzbtm, MPI_REAL8, &
                 pid_imr,251,comm_worker,status(1),ierr)
   call MPIX_RECV_offset_C(v2zD(0,nx2p1,ny2p1),nzbtm, MPI_REAL8, &
                pid_imr,261,comm_worker,status(1),ierr)
#endif
   if(DETAILED_TIMING .eq. 1) then; call log_comm ("v1x recvs", pid_imr, nnum); end if
   if(DETAILED_TIMING .eq. 1) then; call log_comm ("v1y recvs", pid_imr, nnum); end if
   if(DETAILED_TIMING .eq. 1) then; call log_comm ("v1z recvs", pid_imr, nnum); end if
   if(DETAILED_TIMING .eq. 1) then; call log_comm ("v2x recvs", pid_imr, nnum); end if
   if(DETAILED_TIMING .eq. 1) then; call log_comm ("v2y recvs", pid_imr, nnum); end if
   if(DETAILED_TIMING .eq. 1) then; call log_comm ("v2z recvs", pid_imr, nnum); end if
case(2)
#if USE_MPIX == 0 || VALIDATE_MODE == 1
   call MPI_RECV(v1x(0,nx1p1,ny1p1),nnum, MPI_REAL8, &
                 pid_imr,21,comm_worker,status,ierr)
   ...
   call MPI_RECV(v2z(0,nx2p1,ny2p1),nzbtm, MPI_REAL8, &
                 pid_imr,26,comm_worker,status,ierr)
   call MPI_SEND(v1x(0,1,1), nnum, MPI_REAL8, &
                 pid_ims, 21, comm_worker, ierr)
   call MPI_SEND(v1y(0,1,1), nnum, MPI_REAL8, &
                 pid_ims, 22, comm_worker, ierr)
   call MPI_SEND(v1z(0,1,1), nnum, MPI_REAL8, &
                 pid_ims, 23, comm_worker, ierr)
   call MPI_SEND(v2x(0,1,1), nzbtm, MPI_REAL8, &
                 pid_ims, 24, comm_worker, ierr)
   call MPI_SEND(v2y(0,1,1), nzbtm, MPI_REAL8, &
                 pid_ims, 25, comm_worker, ierr)
   call MPI_SEND(v2z(0,1,1), nzbtm, MPI_REAL8, &
   ...
   call MPIX_RECV_offset_C(v2zD(0,nx2p1,ny2p1),nzbtm, MPI_REAL8, &
                 pid_imr,261,comm_worker,status(1),ierr)
   call MPIX_SEND_offset_C(v1xD(0,1,1), nnum, MPI_REAL8, &
                 pid_ims, 211, comm_worker, ierr)
   call MPIX_SEND_offset_C(v1yD(0,1,1), nnum, MPI_REAL8, &
                 pid_ims, 221, comm_worker, ierr)
   call MPIX_SEND_offset_C(v1zD(0,1,1), nnum, MPI_REAL8, &
                 pid_ims, 231, comm_worker, ierr)
   call MPIX_SEND_offset_C(v2xD(0,1,1), nzbtm, MPI_REAL8, &
                 pid_ims, 241, comm_worker, ierr)
   call MPIX_SEND_offset_C(v2yD(0,1,1), nzbtm, MPI_REAL8, &
                 pid_ims, 251, comm_worker, ierr)
   call MPIX_SEND_offset_C(v2zD(0,1,1), nzbtm, MPI_REAL8, &
                 pid_ims, 261, comm_worker, ierr)
#endif
   if(DETAILED_TIMING .eq. 1) then; call log_comm ("v1x recvs", pid_imr, nnum); end if
   if(DETAILED_TIMING .eq. 1) then; call log_comm ("v1y recvs", pid_imr, nnum); end if
   if(DETAILED_TIMING .eq. 1) then; call log_comm ("v1z recvs", pid_imr, nnum); end if
   ...
   if(DETAILED_TIMING .eq. 1) then; call log_comm ("v2y recvs", pid_imr, nnum); end if
   if(DETAILED_TIMING .eq. 1) then; call log_comm ("v2z recvs", pid_imr, nnum); end if
   if(DETAILED_TIMING .eq. 1) then; call log_comm ("v1x sends", pid_imr, nnum); end if
   if(DETAILED_TIMING .eq. 1) then; call log_comm ("v1y sends", pid_imr, nnum); end if
   if(DETAILED_TIMING .eq. 1) then; call log_comm ("v1z sends", pid_imr, nnum); end if
   if(DETAILED_TIMING .eq. 1) then; call log_comm ("v2x sends", pid_imr, nnum); end if
   if(DETAILED_TIMING .eq. 1) then; call log_comm ("v2y sends", pid_imr, nnum); end if
   if(DETAILED_TIMING .eq. 1) then; call log_comm ("v2z sends", pid_imr, nnum); end if
...
#if USE_MPIX==1 && LOGGING_ENABLED == 1
  call log_arr_info(v1x_meta)
  call log_arr_info(v1y_meta)
  call log_arr_info(v1z_meta)
  call log_arr_info(v2x_meta)
  call log_arr_info(v2y_meta)
  call log_arr_info(v2z_meta)
#endif
#if USE_MPIX==1 && VALIDATE_MODE ==1
  call validate(v1x_meta)
  call validate(v1y_meta)
  call validate(v1z_meta)
  call validate(v2x_meta)
  call validate(v2y_meta)
  call validate(v2z_meta)
#endif
...
! SEND
 if(neighb(1) > -1) then
   ...
   if(DETAILED_TIMING .eq. 1) then; call record_time(cpu_tend); call log_timing ("CPU vxy_image_layer_sdx_vel", cpu_tend-cpu_tstart);  &
        total_cpu_marsh_time = total_cpu_marsh_time + (cpu_tend-cpu_tstart); end if
   call validate(sdx1_meta)
   call validate(sdx2_meta)
#endif   
   nr=nr+1

#if USE_MPIX==0 || VALIDATE_MODE == 1 
  call MPI_ISEND(sdx1(1), nytop, MPI_REAL8, &
                  neighb(1), 11, comm_worker, req(nr), ierr)
#endif                  
#if USE_MPIX==1
  call MPIX_ISEND_C(c_sdx1_id, nytop, MPI_REAL8, &
                  neighb(1), 111, comm_worker, reqX(nr), ierr)
#endif
   if(DETAILED_TIMING .eq. 1) then; call log_comm ("sdx1", neighb(1), nytop); end if

   nr=nr+1

#if USE_MPIX==0 || VALIDATE_MODE == 1 
   call MPI_ISEND(sdx2(1), nytop, MPI_REAL8, &
                  neighb(1), 12, comm_worker, req(nr), ierr)
#endif                  
#if USE_MPIX==1
   call MPIX_ISEND_C(c_sdx2_id, nytop, MPI_REAL8, &
                  neighb(1), 121, comm_worker, reqX(nr), ierr)
#endif
   if(DETAILED_TIMING .eq. 1) then; call log_comm ("sdx2", neighb(1), nytop); end if
 endif
 if(neighb(3) > -1) then
    if(DETAILED_TIMING .eq. 1) then;  call record_time(gpu_tstart) ; end if
   ...
   if(DETAILED_TIMING .eq. 1) then; call record_time(cpu_tend); call log_timing ("CPU vxy_image_layer_sdy_vel", cpu_tend-cpu_tstart);  &
        total_cpu_marsh_time = total_cpu_marsh_time + (cpu_tend-cpu_tstart); end if
   call validate(sdy1_meta)
   call validate(sdy2_meta)
#endif   

   nr=nr+1

#if USE_MPIX==0 || VALIDATE_MODE == 1 
   call MPI_ISEND(sdy1(1), nxtop, MPI_REAL8, &
                  neighb(3), 33, comm_worker, req(nr), ierr)
#endif                  
#if USE_MPIX==1
   call MPIX_ISEND_C(c_sdy1_id, nxtop, MPI_REAL8, &
                  neighb(3), 331, comm_worker, reqX(nr), ierr)
#endif
   if(DETAILED_TIMING .eq. 1) then; call log_comm ("sdy1", neighb(3), nxtop); end if

   nr=nr+1
#if USE_MPIX==0 || VALIDATE_MODE == 1 
   call MPI_ISEND(sdy2(1), nxtop, MPI_REAL8, &
                  neighb(3), 34, comm_worker, req(nr), ierr)
#endif                  
#if USE_MPIX==1
   call MPIX_ISEND_C(c_sdy2_id, nxtop, MPI_REAL8, &
                  neighb(3), 341, comm_worker, reqX(nr), ierr)
#endif
   if(DETAILED_TIMING .eq. 1) then; call log_comm ("sdy2", neighb(3), nxtop); end if
 endif
! RECV
 if(neighb(2) > -1) then
   nr=nr+1
#if USE_MPIX==0 || VALIDATE_MODE == 1 
   call MPI_IRECV(rcx1(1), nytop, MPI_REAL8, &
                  neighb(2), 11, comm_worker, req(nr), ierr)
#endif                  
#if USE_MPIX==1
   call MPIX_IRECV_C(c_rcx1_id, nytop, MPI_REAL8, &
                  neighb(2), 111, comm_worker, reqX(nr), ierr)
#endif
   if(DETAILED_TIMING .eq. 1) then; call log_comm ("rcx1", neighb(2), nytop); end if
   nr=nr+1
#if USE_MPIX==0 || VALIDATE_MODE == 1 
   call MPI_IRECV(rcx2(1), nytop, MPI_REAL8, &
                  neighb(2), 12, comm_worker, req(nr), ierr)
#endif                  
#if USE_MPIX==1
   call MPIX_IRECV_C(c_rcx2_id, nytop, MPI_REAL8, &
                  neighb(2), 121, comm_worker, reqX(nr), ierr)
#endif
   if(DETAILED_TIMING .eq. 1) then; call log_comm ("rcx2", neighb(2), nytop); end if
 endif
 if(neighb(4) > -1) then
   nr=nr+1
#if USE_MPIX==0 || VALIDATE_MODE == 1 
   call MPI_IRECV(rcy1(1), nxtop, MPI_REAL8, &
                  neighb(4), 33, comm_worker, req(nr), ierr)
#endif                  
#if USE_MPIX==1
   call MPIX_IRECV_C(c_rcy1_id, nxtop, MPI_REAL8, &
                  neighb(4), 331, comm_worker, reqX(nr), ierr)
#endif
   if(DETAILED_TIMING .eq. 1) then; call log_comm ("rcy1", neighb(4), nytop); end if

   nr=nr+1
#if USE_MPIX==0 || VALIDATE_MODE == 1 
   call MPI_IRECV(rcy2(1), nxtop, MPI_REAL8, &
                  neighb(4), 34, comm_worker, req(nr), ierr)
#endif                  
#if USE_MPIX==1
   call MPIX_IRECV_C(c_rcy2_id, nxtop, MPI_REAL8, &
                  neighb(4), 341, comm_worker, reqX(nr), ierr)
#endif
   if(DETAILED_TIMING .eq. 1) then; call log_comm ("rcy2", neighb(4), nytop); end if
 endif
...
#if USE_MPIX==1 && LOGGING_ENABLED == 1
 call log_arr_info(rcx1_meta)
 call log_arr_info(rcx2_meta)
 call log_arr_info(rcy1_meta)
 call log_arr_info(rcy2_meta)
#endif
#if USE_MPIX==1 && VALIDATE_MODE == 1
 call validate(rcx1_meta)
 call validate(rcx2_meta)
 call validate(rcy1_meta)
 call validate(rcy2_meta)
#endif
 ...
 if(neighb(2) > -1) then
   ...
   if(DETAILED_TIMING .eq. 1) then; call record_time(cpu_tend); call log_timing ("CPU vxy_image_layer_rcx_vel", cpu_tend-cpu_tstart);  &
        total_cpu_marsh_time = total_cpu_marsh_time + (cpu_tend-cpu_tstart); end if
   call validate(v1x_meta)
   call validate(v1y_meta)
#endif   
 ...
 if(neighb(4) > -1) then
   ...
   if(DETAILED_TIMING .eq. 1) then; call record_time(cpu_tend); call log_timing ("CPU vxy_image_layer_rcx2_vel", cpu_tend-cpu_tstart);  &
        total_cpu_marsh_time = total_cpu_marsh_time + (cpu_tend-cpu_tstart); end if
   call validate(v1x_meta)
   call validate(v1y_meta)
#endif   
 ...
 if(DETAILED_TIMING .eq. 1) then; call log_timing ("GPU Marshaling vxy_image_layer", total_gpu_marsh_time); end if
#if VALIDATE_MODE == 1
 if(DETAILED_TIMING .eq. 1) then; call log_timing ("CPU Marshaling vxy_image_layer", total_cpu_marsh_time); end if
#endif
...
!  Compute the velocity at region I
 ...
 implicit NONE
 integer:: i,j,k,k3
 double precision:: dtxz,dtyz,dtzz
!
...
!  Compute the velocities in region of PML-x-I
 ...
 double precision:: rth,rti,damp0,dmpx2,dmpx1,dmpyz2,dmpyz1,ro1,rox,roy,roz, &
...
!   Compute the velocities in region of PML-y-I
 ...
 double precision:: rth,rti,damp0,dmpy2,dmpy1,dmpxz2,dmpxz1,ro1,rox,roy,roz, &
...
! Compute the velocities in region of PML-z-II
 ...
 implicit NONE
 integer:: i,j,k,kb
 double precision:: damp0,dmpz2,dmpz1,dmpxy2,dmpxy1,ro1,rox,roy,roz, &
        vtmpx,vtmpy,vtmpz
!
 do j=nd2_vel(1),nd2_vel(6)
 do i=nd2_vel(7),nd2_vel(12)
...
subroutine comm_stress(myid, comm_worker)
 use iso_c_binding
 use grid_node_comm
 use wave_field_comm
 use metadata
 ...
 integer, intent(IN):: myid, comm_worker
 integer::ierr
 real(c_double) :: tstart, tend
! Interpolating Stress on the interface betwee Region I and II
 call record_time(tstart)
 call stress_interp
 call record_time(tend)
 call log_timing('stress_interp', tend-tstart)

 call record_time(tstart)
 call ISEND_IRECV_Stress(comm_worker)
 call record_time(tend)
 call log_timing('ISEND_IRECV_Stress', tend-tstart)
...
! Adding double couple sources 
 ...
 double precision, intent(IN):: tim
 integer:: i,j,k,kap
 double precision::sutm,fsource
 double precision, allocatable, target, dimension(:) :: sutmArr
 real(c_double) :: gpu_tstart, gpu_tend, cpu_tstart, cpu_tend
 real(c_double):: total_gpu_marsh_time=0.0, total_cpu_marsh_time=0.0
 total_gpu_marsh_time =0.0
 total_cpu_marsh_time =0.0
...
#if LOGGING_ENABLED == 1  
 call log_string ("---------- SUBROUTINE add_dcs_vel")
#endif 
...
! Myz
 ...
 enddo
 if(DETAILED_TIMING .eq. 1) then; call record_time(cpu_tend); call log_timing ("CPU add_dcs", cpu_tend-cpu_tstart); &
        total_cpu_marsh_time = total_cpu_marsh_time + (cpu_tend-cpu_tstart); end if
 call validate(t1xx_meta)
 call validate(t1yy_meta)
 call validate(t1zz_meta)
 call validate(t2xx_meta)
 call validate(t2yy_meta)
 call validate(t2zz_meta)
 call validate(t1xy_meta)
 call validate(t2xy_meta)
 call validate(t1xz_meta)
 call validate(t2xz_meta)
 call validate(t1yz_meta)
 call validate(t2yz_meta)
#endif

 if(DETAILED_TIMING .eq. 1) then; call log_timing ("GPU Marshaling add_dcs", total_gpu_marsh_time); end if
#if VALIDATE_MODE == 1
 if(DETAILED_TIMING .eq. 1) then; call log_timing ("CPU Marshaling add_dcs", total_cpu_marsh_time); end if
#endif
 return
...
!  When you modify this function please check normalizinf coefticient
 implicit NONE
 double precision, parameter:: pi=3.1415926, pi2=2.*pi, pi25=2.5*pi
 double precision, parameter:: cft4=0.20, c41=0.60*0.5, c42=0.5-c41, c43=2.*c42
 double precision, parameter:: cft5=0.20, c51=0.50*0.5, c52=0.5-c51
 integer, intent(IN):: id_sf_type
 double precision, intent(IN):: t,dura,sp2
 double precision:: tt1,tt2, fsource
! 
 fsource= 0.0
...
! Compute stress-Norm and XY component in Region I
 ...
 double precision:: sxx,syy,szz,sxy,qxx,qyy,qzz,qxy,cusxy,sss, &
...
! Compute stress-XZand YZ component in Region I
 ...
 use wave_field_comm
 implicit NONE
 double precision, parameter:: tfr1=-577./528./ca,tfr2=201./176./ca, &
                   tfr3=-9./176./ca,  tfr4=1./528./ca
 integer:: i,j,k,kodd,inod,jkq,irw
 double precision:: dvzx,dvzy,dvxz,dvyz,sm,cusxz,cusyz,et,et1,dmws,qxz,qyz
!
...
! Compute stress-Norm and XY component in Region II
 ...
 implicit NONE
 integer:: i,j,k,kodd,inod,jkq,irw
 double precision:: sxx,syy,szz,sxy,sss,qxx,qyy,qzz,qxy,cusxy, &
...
! Compute stress-XZ and YZ component in the Region II
 ...
 implicit NONE
 integer:: i,j,k,kodd,inod,jkq,irw
 double precision:: qxz,qyz,cusxz,cusyz,sm,et,et1,dmws
!
...
! Compute the velocity of PML-x-I region
 ...
 double precision:: taoxx,taoyy,taozz,sxx,syy,szz,sss,qxx,qyy,qzz, &
...
! Compute the Stress-xy at region of PML-x-I
 ...
 implicit NONE
 integer:: i,j,k,lb,ib,kb,kodd,jkq,inod,irw
 double precision:: taoxy,cusxy,qxy,rth,damp2,damp1,sm,dmws,et,et1
!
 if ( lbx(1)>lbx(2) ) return
...
! Compute the Stress-xy at region of PML-y-I
 ...
 implicit NONE
 integer:: i,j,k,lb,jb,kb,kodd,jkq,inod,irw
 double precision:: taoxy,cusyx,qxy,rth,damp2,damp1,sm,dmws,et,et1
!
 if( lby(1)>lby(2) ) return
 jb=0
 do lb=lby(1), lby(2)
...
! Compute the stress-xz at PML-x-I region
 ...
 implicit NONE
 integer:: i,j,k,lb,ib,kb,kodd,jkq,inod,irw
 double precision:: taoxz,cusxz,dvxz,qxz,rth,damp2,damp1,sm,dmws,et,et1
!
 if ( lbx(1)>lbx(2) ) return
...
! Compute the stress-xz at PML-y-I region
 ...
 double precision:: cusxz,dvxz,dvzx,qxz,sm,dmws,et,et1
!
 if( lby(1)>lby(2) ) return
 do lb=lby(1),lby(2)
...
! Compute the stress-yz at PML-x-I region
 ...
 implicit NONE
 integer:: i,j,k,lb,kodd,jkq,inod,irw
 double precision:: cusyz,dvyz,dvzy,qyz,sm,dmws,et,et1
!
 if( lbx(1)>lbx(2) ) return
 do j=nd1_tyz(3),nd1_tyz(4)
   kodd=2*mod(j+nyb1,2)+1
...
! Compute the stress-yz at PML-y-I region
 ...
 implicit NONE
 integer:: i,j,k,lb,jb,kb,kodd,jkq,inod,irw
 double precision:: taoyz,cusyz,dvyz,qyz,rth,damp2,damp1,sm,dmws,et,et1
!
 if( lby(1)>lby(2) ) return
 jb=0
 do lb=lby(1),lby(2)
...
!  Compute the Stress-xy at region of PML-z-II
 ...
 implicit NONE
 integer:: i,j,k,lb,kodd,jkq,inod,irw
 double precision:: cusxy,qxy,sm,dmws,et,et1
...
! Compute the stress-xz at region of PML-x-II
 ...
 implicit NONE
 integer:: i,j,k,lb,ib,kb,kodd,jkq,inod,irw
 double precision:: taoxz,cusxz,qxz,rth,damp2,damp1,sm,dmws,et,et1
!
 if( lbx(1)>lbx(2) ) return
...
! Compute the stress-xz at region of PML-y-II
 ...
 implicit NONE
 integer:: i,j,k,lb,kodd,jkq,inod,irw
 double precision:: dvxz,dvzx,cusxz,qxz,sm,dmws,et,et1
!
 if( lby(1)>lby(2) ) return
 do lb=lby(1),lby(2)
...
! Compute the stress-xz at region of PML-z-II
 ...
 implicit NONE
 integer:: i,j,k,lb,kb,kodd,jkq,inod,irw
 double precision:: taoxz,cusxz,qxz,damp2,damp1,sm,dmws,et,et1
!
 do j=nd2_txz(1),nd2_txz(6)
   kodd=2*mod(j+nyb2,2)+1
...
! Compute the stress-yz at region of PML-x-II
 ...
 implicit NONE
 integer:: i,j,k,lb,kodd,jkq,inod,irw
 double precision:: cusyz,qyz,sm,dmws,et,et1
!
 if( lbx(1)>lbx(2) ) return
 do j=nd2_tyz(3),nd2_tyz(4)
   kodd=2*mod(j+nyb2,2)+1
...
! Compute the stress-yz at region of PML-y-II
 ...
 implicit NONE
 integer:: i,j,k,lb,jb,kb,kodd,jkq,inod,irw
 double precision:: taoyz,cusyz,qyz,rth,damp2,damp1,sm,dmws,et,et1
!
 if( lby(1)>lby(2) ) return
 jb=0
 do lb=lby(1),lby(2)
...
! Compute the stress-yz at region of PML-y-II
 ...
 implicit NONE
 integer:: i,j,k,lb,kb,kodd,jkq,inod,irw
 double precision:: taoyz,cusyz,qyz,damp2,damp1,sm,dmws,et,et1
!
...
subroutine stress_interp
 ...
 real(c_double):: total_gpu_marsh_time=0.0, total_cpu_marsh_time=0.0
#if LOGGING_ENABLED == 1  
 call log_string("----------SUBROUTINE interp_stress")
#endif
 total_gpu_marsh_time =0.0
 total_cpu_marsh_time =0.0

! call interp_stress (neighb(1), neighb(2), neighb(3), neighb(4) ,&
!                    nxbm1, nybm1, nxbtm, nybtm, nzbtm, nxtop, nytop, nztop, nz1p1)
 ntx1=nxbtm
...
j=-2
 do jj=1,nybtm
   ...
   enddo
 enddo
 if(DETAILED_TIMING .eq. 1) then; call record_time(cpu_tend); call log_timing ("CPU stress_interp1", cpu_tend-cpu_tstart);  &
        total_cpu_marsh_time = total_cpu_marsh_time + (cpu_tend-cpu_tstart); end if
 call validate(t2xz_meta)
!!
 if(DETAILED_TIMING .eq. 1) then;  call record_time(cpu_tstart) ; end if
 ...
 do jj=1,nty1
   ...
   enddo
 enddo
 if(DETAILED_TIMING .eq. 1) then; call record_time(cpu_tend); call log_timing ("CPU stress_interp2", cpu_tend-cpu_tstart);  &
        total_cpu_marsh_time = total_cpu_marsh_time + (cpu_tend-cpu_tstart); end if
 call validate(t2yz_meta)
!!
 if(DETAILED_TIMING .eq. 1) then;  call record_time(cpu_tstart) ; end if
 ...
 do jj=1,nybtm
   ...
   enddo
 enddo
 if(DETAILED_TIMING .eq. 1) then; call record_time(cpu_tend); call log_timing ("CPU stress_interp3", cpu_tend-cpu_tstart);  &
        total_cpu_marsh_time = total_cpu_marsh_time + (cpu_tend-cpu_tstart); end if
 call validate(t2zz_meta)
#endif
...
#if VALIDATE_MODE == 1
  if(DETAILED_TIMING .eq. 1) then; call log_timing ("CPU Marshaling stress_interp", total_cpu_marsh_time); end if
#endif
...
! output the simulation at each time step
 ...
 double precision, dimension(nrev3), intent(OUT):: vout
 integer:: ir,io,i,j,k
 double precision::rx1,rx11,rx2,rx21,ry1,ry11,ry2,ry21,rz1,rz11,rz2,rz21 
!
 do ir=1,nrecs
   rx1=xcor(1,ir)
   rx11=1.-rx1
...
! compute the constants parameters
 ...
 implicit NONE
 integer:: nm
 double precision:: vp,vs,dens,sm,cl,prym,bm
 do nm=1,nmat
   vp=clamda(nm)
   vs=cmu(nm)
   dens=rho(nm)
...
! Determine the parameters of absorbing boundary condition
 implicit NONE
 double precision, intent(IN):: vs,gridsp,dt,damp0
 double precision, dimension(4), intent(OUT):: cfabc
 double precision:: vp,ss,ddd,damp
!
 damp=0.1*damp0
 vp=1.73*vs
 vp=vs
...
subroutine assign_source(xs,ys,zs,ix2,jy2,kz2,cf8,nsou)
 ...
 double precision, dimension(72,4), intent(OUT):: cf8
 integer:: i,j,k,n1,n2,nx,ny,nsi,ijk,kzdf,knd,khf,ind,ihf,jnd,jhf
 integer, dimension(4):: nss
 double precision, dimension(2):: xrt,yrt,zrt,xrh,yrh,zrh
!
 kzdf=nztop
...
! ijk=1, normal; =2, Txy; =3, Txz; =4 Tyz;
 ...
 double precision, dimension(2), intent(IN):: xrt,yrt,zrt
 double precision, dimension(72), intent(OUT)::cf8 
...
subroutine source_index(kzdf,nn,dist,xs,ind,rnd,ihf,rhf)
 ...
 double precision, intent(IN):: xs,dist(nn+6)
 double precision, intent(OUT):: rnd,rhf
 integer:: n1,n2,nm,ii
! 
 n1=2
 n2=nn+4
...
subroutine set_receiver(name_stn,ir,xr,yr,zr)
 ...
 double precision, intent(IN):: xr,yr,zr
!
...
subroutine receiver_index(nn,dist,xs,nr2,xr2)
 ...
 double precision, intent(IN):: dist(nn+6),xs
 double precision, intent(OUT):: xr2(2)
...
subroutine eq_linear(imd,is_hf,ndm,dist,dt,diff_op)
 ...
 double precision, intent(IN):: dt
 double precision, dimension(ndm), intent(IN):: dist
 double precision, dimension(4), intent(OUT):: diff_op
 integer:: i,j,k,n,ii,imax
 double precision:: cff(4),aa(4,4),bb(4),amx,summ,pc
!
 if(is_hf==0) then
   ii=imd+2
...
subroutine BSL_interp(x,h,cf)
 ...
 double precision,dimension(3), intent(IN):: h
 double precision, dimension(4), intent(OUT):: cf
 integer:: i,j
 double precision:: y(4),rh(2)
 double precision:: dy21,dy32,dy43,d2y1,d2y2,ss,rx
!
 rx=x/h(2)
 rh(1)=h(1)/h(2)
 rh(2)=h(3)/h(2)
...
#if LOGGING_ENABLED == 1  
 call log_string("----------SUBROUTINE ISEND_IRECV_Stress")
#endif
  total_gpu_marsh_time =0.0
 total_cpu_marsh_time =0.0
...
!-from 1 to 2
 if(neighb(1)>-1) then
   ...
   if(DETAILED_TIMING .eq. 1) then; call record_time(cpu_tend); call log_timing ("CPU sdx41_stress", cpu_tend-cpu_tstart); &
        total_cpu_marsh_time = total_cpu_marsh_time + (cpu_tend-cpu_tstart); end if
   call validate(sdx41_meta)
#endif   
   nr=nr+1
   ncout(nr)=4*nztop*nytop

#if USE_MPIX == 0 || VALIDATE_MODE == 1
   call MPI_ISEND(sdx41, ncout(nr), MPI_REAL8, & 
                  neighb(1), 501, comm_worker, req(nr), ierr)
#endif                  
#if USE_MPIX == 1
   call MPIX_ISEND_C(c_sdx41_id, ncout(nr), MPI_REAL8, &
                  neighb(1), 5011, comm_worker, reqX(nr), ierr)
#endif
   if(DETAILED_TIMING .eq. 1) then; call log_comm ("sdx41", neighb(1), ncout(nr)); end if
   ...
   if(DETAILED_TIMING .eq. 1) then; call record_time(cpu_tend); call log_timing ("CPU sdx42_stress", cpu_tend-cpu_tstart); &
        total_cpu_marsh_time = total_cpu_marsh_time + (cpu_tend-cpu_tstart); end if

   call validate(sdx42_meta)
#endif   
   nr=nr+1
   ncout(nr)=4*nzbtm*nybtm
#if USE_MPIX == 0 || VALIDATE_MODE == 1
   call MPI_ISEND(sdx42, ncout(nr), MPI_REAL8, & 
                  neighb(1), 502, comm_worker, req(nr), ierr)
#endif                  
#if USE_MPIX == 1
   call MPIX_ISEND_C(c_sdx42_id, ncout(nr), MPI_REAL8, & 
                  neighb(1), 5021, comm_worker, reqX(nr), ierr)
#endif
   if(DETAILED_TIMING .eq. 1) then; call log_comm ("sdx42", neighb(1), ncout(nr)); end if
 endif
!-from 2 to 1
 if(neighb(2)>-1) then
   ...
   if(DETAILED_TIMING .eq. 1) then; call record_time(cpu_tend); call log_timing ("CPU sdx51_stress", cpu_tend-cpu_tstart); &
        total_cpu_marsh_time = total_cpu_marsh_time + (cpu_tend-cpu_tstart); end if
   call validate(sdx51_meta)
#endif
   nr=nr+1
   ncout(nr)=5*nztop*nytop
#if USE_MPIX == 0 || VALIDATE_MODE == 1
   call MPI_ISEND(sdx51, ncout(nr), MPI_REAL8, & 
                  neighb(2), 601, comm_worker, req(nr), ierr)
#endif                  
#if USE_MPIX == 1
   call MPIX_ISEND_C(c_sdx51_id, ncout(nr), MPI_REAL8, & 
                  neighb(2), 6011, comm_worker, reqX(nr), ierr)
#endif
   if(DETAILED_TIMING .eq. 1) then; call log_comm ("sdx51", neighb(2), ncout(nr)); end if
   ...
   call validate(sdx52_meta)
   if(DETAILED_TIMING .eq. 1) then; call record_time(cpu_tend); call log_timing ("CPU sdx52_stress", cpu_tend-cpu_tstart); &
        total_cpu_marsh_time = total_cpu_marsh_time + (cpu_tend-cpu_tstart); end if
#endif
   nr=nr+1
   ncout(nr)=5*nzbtm*nybtm
#if USE_MPIX == 0 || VALIDATE_MODE == 1
  call MPI_ISEND(sdx52, ncout(nr), MPI_REAL8, &
                  neighb(2), 602, comm_worker, req(nr), ierr)
#endif                  
#if USE_MPIX == 1
  call MPIX_ISEND_C(c_sdx52_id, ncout(nr), MPI_REAL8, &
                  neighb(2), 6021, comm_worker, reqX(nr), ierr)
#endif
   if(DETAILED_TIMING .eq. 1) then; call log_comm ("sdx52", neighb(2), ncout(nr)); end if
 endif
!-from 3 to 4
 if(neighb(3)>-1) then
   ...
   if(DETAILED_TIMING .eq. 1) then; call record_time(cpu_tend); call log_timing ("CPU sdy41_stress", cpu_tend-cpu_tstart); &
        total_cpu_marsh_time = total_cpu_marsh_time + (cpu_tend-cpu_tstart); end if

   call validate(sdy41_meta)
#endif
   nr=nr+1
   ncout(nr)=4*nztop*nxtop
#if USE_MPIX == 0 || VALIDATE_MODE == 1
 call MPI_ISEND(sdy41, ncout(nr), MPI_REAL8, &
                  neighb(3), 701, comm_worker, req(nr), ierr)
#endif                  
#if USE_MPIX == 1
 call MPIX_ISEND_C(c_sdy41_id, ncout(nr), MPI_REAL8, &
                  neighb(3), 7011, comm_worker, reqX(nr), ierr)
#endif
   if(DETAILED_TIMING .eq. 1) then; call log_comm ("sdy41", neighb(3), ncout(nr)); end if
  ...
  sdy42(:, :, 4)=t2yz(1:nzbtm, 1:nxbtm, 1)
   if(DETAILED_TIMING .eq. 1) then; call record_time(cpu_tend); call log_timing ("CPU sdy42_stress", cpu_tend-cpu_tstart); &
        total_cpu_marsh_time = total_cpu_marsh_time + (cpu_tend-cpu_tstart); end if

  call validate(sdy42_meta)
#endif
   nr=nr+1
   ncout(nr)=4*nzbtm*nxbtm
#if USE_MPIX == 0 || VALIDATE_MODE == 1
    call MPI_ISEND(sdy42, ncout(nr), MPI_REAL8, &
                  neighb(3), 702, comm_worker, req(nr), ierr)
#endif                  
#if USE_MPIX == 1
    call MPIX_ISEND_C(c_sdy42_id, ncout(nr), MPI_REAL8, &
                  neighb(3), 7021, comm_worker, reqX(nr), ierr)
#endif
   if(DETAILED_TIMING .eq. 1) then; call log_comm ("sdy42", neighb(3), ncout(nr)); end if
 endif
!-from 4 to 3
 if(neighb(4)>-1) then
   ...
   if(DETAILED_TIMING .eq. 1) then; call record_time(cpu_tend); call log_timing ("CPU sdy51_stress", cpu_tend-cpu_tstart); &
        total_cpu_marsh_time = total_cpu_marsh_time + (cpu_tend-cpu_tstart); end if

   call validate(sdy51_meta)
#endif
   nr=nr+1
   ncout(nr)=5*nztop*nxtop
#if USE_MPIX == 0 || VALIDATE_MODE == 1
    call MPI_ISEND(sdy51, ncout(nr), MPI_REAL8, &
                  neighb(4), 801, comm_worker, req(nr), ierr)
#endif    
#if USE_MPIX == 1
    call MPIX_ISEND_C(c_sdy51_id, ncout(nr), MPI_REAL8, &
                  neighb(4), 8011, comm_worker, reqX(nr), ierr)
#endif
   if(DETAILED_TIMING .eq. 1) then; call log_comm ("sdy51", neighb(4), ncout(nr)); end if
   ...
   if(DETAILED_TIMING .eq. 1) then; call record_time(cpu_tend); call log_timing ("CPU sdy52_stress", cpu_tend-cpu_tstart); &
        total_cpu_marsh_time = total_cpu_marsh_time + (cpu_tend-cpu_tstart); end if

   call validate(sdy52_meta)
#endif

   nr=nr+1
   ncout(nr)=5*nzbtm*nxbtm
#if USE_MPIX == 0 || VALIDATE_MODE == 1
 call MPI_ISEND(sdy52, ncout(nr), MPI_REAL8, &
                  neighb(4), 802, comm_worker, req(nr), ierr)
#endif                  
#if USE_MPIX == 1
 call MPIX_ISEND_C(c_sdy52_id, ncout(nr), MPI_REAL8, &
                  neighb(4), 8021, comm_worker, reqX(nr), ierr)
#endif
   if(DETAILED_TIMING .eq. 1) then; call log_comm ("sdy52", neighb(4), ncout(nr)); end if
 endif
!
! Receive
!-from 1 to 2
 if(neighb(2)>-1) then
   ...
   ncout(nr)=4*nztop*nytop
#if USE_MPIX == 0 || VALIDATE_MODE == 1
  call MPI_IRECV(rcx41, ncout(nr), MPI_REAL8, &
                  neighb(2), 501, comm_worker, req(nr), ierr)
#endif                  
#if USE_MPIX == 1
  call MPIX_IRECV_C(c_rcx41_id, ncout(nr), MPI_REAL8, &
                  neighb(2), 5011, comm_worker, reqX(nr), ierr)
#endif
   if(DETAILED_TIMING .eq. 1) then; call log_comm ("rcx41", neighb(2), ncout(nr)); end if
   nr=nr+1
   ncout(nr)=4*nzbtm*nybtm
#if USE_MPIX == 0 || VALIDATE_MODE == 1
 call MPI_IRECV(rcx42, ncout(nr), MPI_REAL8, &
                  neighb(2), 502, comm_worker, req(nr), ierr)
#endif                  
#if USE_MPIX == 1
 call MPIX_IRECV_C(c_rcx42_id, ncout(nr), MPI_REAL8, &
                  neighb(2), 5021, comm_worker, reqX(nr), ierr)
#endif
   if(DETAILED_TIMING .eq. 1) then; call log_comm ("rcx42", neighb(2), ncout(nr)); end if
 endif
!-from 2 to 1
 if(neighb(1)>-1) then
   nr=nr+1
   ncout(nr)=5*nztop*nytop
#if USE_MPIX == 0 || VALIDATE_MODE == 1
    call MPI_IRECV(rcx51, ncout(nr), MPI_REAL8, &
                  neighb(1), 601, comm_worker, req(nr), ierr)
#endif                  
#if USE_MPIX == 1
    call MPIX_IRECV_C(c_rcx51_id, ncout(nr), MPI_REAL8, &
                  neighb(1), 6011, comm_worker, reqX(nr), ierr)
#endif
   if(DETAILED_TIMING .eq. 1) then; call log_comm ("rcx51", neighb(1), ncout(nr)); end if
   nr=nr+1
   ncout(nr)=5*nzbtm*nybtm
#if USE_MPIX == 0 || VALIDATE_MODE == 1
    call MPI_IRECV(rcx52, ncout(nr), MPI_REAL8, &
                  neighb(1), 602, comm_worker, req(nr), ierr)
#endif    
#if USE_MPIX == 1
    call MPIX_IRECV_C(c_rcx52_id, ncout(nr), MPI_REAL8, &
                  neighb(1), 6021, comm_worker, reqX(nr), ierr)
#endif
   if(DETAILED_TIMING .eq. 1) then; call log_comm ("rcx52", neighb(1), ncout(nr)); end if
 endif
!-from 3 to 4
 if(neighb(4)>-1) then
   nr=nr+1
   ncout(nr)=4*nztop*nxtop
#if USE_MPIX == 0 || VALIDATE_MODE == 1
    call MPI_IRECV(rcy41, ncout(nr), MPI_REAL8, &
                  neighb(4), 701, comm_worker, req(nr), ierr)
#endif                  
#if USE_MPIX == 1
    call MPIX_IRECV_C(c_rcy41_id, ncout(nr), MPI_REAL8, &
                  neighb(4), 7011, comm_worker, reqX(nr), ierr)
#endif
   if(DETAILED_TIMING .eq. 1) then; call log_comm ("rcy41", neighb(4), ncout(nr)); end if
   nr=nr+1
   ncout(nr)=4*nzbtm*nxbtm
#if USE_MPIX == 0 || VALIDATE_MODE == 1
    call MPI_IRECV(rcy42, ncout(nr), MPI_REAL8, &
                  neighb(4), 702, comm_worker, req(nr), ierr)
#endif                  
#if USE_MPIX == 1
    call MPIX_IRECV_C(c_rcy42_id, ncout(nr), MPI_REAL8, &
                  neighb(4), 7021, comm_worker, reqX(nr), ierr)
#endif
   if(DETAILED_TIMING .eq. 1) then; call log_comm ("rcy42", neighb(4), ncout(nr)); end if
 endif
!-from 4 to 3
 if(neighb(3)>-1) then
   nr=nr+1
   ncout(nr)=5*nztop*nxtop
#if USE_MPIX == 0 || VALIDATE_MODE == 1
    call MPI_IRECV(rcy51, ncout(nr), MPI_REAL8, &
                  neighb(3), 801, comm_worker, req(nr), ierr)
#endif                  
#if USE_MPIX == 1
    call MPIX_IRECV_C(c_rcy51_id, ncout(nr), MPI_REAL8, &
                  neighb(3), 8011, comm_worker, reqX(nr), ierr)
#endif
   if(DETAILED_TIMING .eq. 1) then; call log_comm ("rcy51", neighb(3), ncout(nr)); end if
   nr=nr+1
   ncout(nr)=5*nzbtm*nxbtm
#if USE_MPIX == 0 || VALIDATE_MODE == 1
    call MPI_IRECV(rcy52, ncout(nr), MPI_REAL8, &
                  neighb(3), 802, comm_worker, req(nr), ierr)
#endif                  
#if USE_MPIX == 1
    call MPIX_IRECV_C(c_rcy52_id, ncout(nr), MPI_REAL8, &
                  neighb(3), 8021, comm_worker, reqX(nr), ierr)
#endif
   if(DETAILED_TIMING .eq. 1) then; call log_comm ("rcy52", neighb(3), ncout(nr)); end if
 endif
...
#if USE_MPIX==1 && LOGGING_ENABLED == 1 
 call log_arr_info(rcx41_meta)
 call log_arr_info(rcx42_meta)
 call log_arr_info(rcy41_meta)
 call log_arr_info(rcy42_meta)
 call log_arr_info(rcx51_meta)
 call log_arr_info(rcx52_meta)
 call log_arr_info(rcy51_meta)
 call log_arr_info(rcy52_meta)
#endif
#if USE_MPIX==1 && VALIDATE_MODE == 1
 call validate(rcx41_meta)
 call validate(rcx42_meta)
 call validate(rcy41_meta)
 call validate(rcy42_meta)
 call validate(rcx51_meta)
 call validate(rcx52_meta)
 call validate(rcy51_meta)
 call validate(rcy52_meta)
#endif
...
!-from 1 to 2
 if(neighb(2)>-1) then
   ...
   if(DETAILED_TIMING .eq. 1) then; call record_time(cpu_tend); call log_timing ("CPU rcx41/2_stress", cpu_tend-cpu_tstart); &
        total_cpu_marsh_time = total_cpu_marsh_time + (cpu_tend-cpu_tstart); end if
...
!-from 2 to 1
 if(neighb(1)>-1) then
   ...
   if(DETAILED_TIMING .eq. 1) then; call record_time(cpu_tend); call log_timing ("CPU rcx51/2_stress", cpu_tend-cpu_tstart); &
        total_cpu_marsh_time = total_cpu_marsh_time + (cpu_tend-cpu_tstart); end if
   call validate(t1xx_meta)
   call validate(t1xy_meta)
   call validate(t1xz_meta)
...
!------------------ from 3 to 4
 if(neighb(4)>-1) then
   ...
   if(DETAILED_TIMING .eq. 1) then; call record_time(cpu_tend); call log_timing ("CPU rcy42_stress", cpu_tend-cpu_tstart); &
        total_cpu_marsh_time = total_cpu_marsh_time + (cpu_tend-cpu_tstart); end if
...
!-from 4 to 3
 if(neighb(3)>-1) then
   ...
   if(DETAILED_TIMING .eq. 1) then; call record_time(cpu_tend); call log_timing ("CPU rcy51/2_stress", cpu_tend-cpu_tstart); &
        total_cpu_marsh_time = total_cpu_marsh_time + (cpu_tend-cpu_tstart); end if
   call validate(t1yy_meta)
   call validate(t1xy_meta)
   call validate(t1yz_meta)
 ...
 if(DETAILED_TIMING .eq. 1) then; call log_timing ("GPU Marshaling ISEND_IRECV_Stress", total_gpu_marsh_time); end if
#if VALIDATE_MODE == 1
 if(DETAILED_TIMING .eq. 1) then; call log_timing ("CPU Marshaling ISEND_IRECV_Stress", total_cpu_marsh_time); end if
#endif
...
!
!-----------------------------------------------------------------------
subroutine ISEND_IRECV_Velocity(myid,comm_worker)
 use mpi
 use grid_node_comm
 use wave_field_comm
 use logging
 ...
 interface
    ...
    subroutine print_3d_array(array_name, array_ptr, size_x, size_y, size_z)
        ...
        real(kind=c_double), pointer, dimension(:,:,:) :: array_ptr
        integer:: i,j,k
 ...
 double precision:: time_marsh_send_ISRV=0.0,time_marsh_recv_ISRV=0.0
 real(c_double) :: gpu_tstart, gpu_tend, cpu_tstart, cpu_tend
 real(c_double) :: total_gpu_marsh_time=0.0, total_cpu_marsh_time=0.0
 type(c_ptr) :: temp_rcx41C
 real(kind=c_double), pointer, dimension(:,:,:) :: rcx41_tmp
 integer(kind=MPI_ADDRESS_KIND) size 
 integer :: real_8_itemsize, ierror

#if LOGGING_ENABLED == 1  
 call log_string("----------SUBROUTINE ISEND_IRECV_Velocity")
#endif
...
!-from 1 to 2
 ...
 if(neighb(1)>-1) then
   ...
   enddo
   enddo
   if(DETAILED_TIMING .eq. 1) then; call record_time(cpu_tend); call log_timing ("CPU sdx51_vel", cpu_tend-cpu_tstart);  &
        total_cpu_marsh_time = total_cpu_marsh_time + (cpu_tend-cpu_tstart); end if
   call validate(sdx51_meta)
#endif   
   nr=nr+1
...
#if USE_MPIX == 1
   call MPIX_ISEND_C(c_sdx51_id, ncout(nr), MPI_REAL8, &
                neighb(1), 1011, comm_worker, reqX(nr), ierr)
#endif
   if(DETAILED_TIMING .eq. 1) then; call log_comm ("sdx51", neighb(1), ncout(nr)); end if
   ...
   enddo
   enddo
   if(DETAILED_TIMING .eq. 1) then; call record_time(cpu_tend); call log_timing ("CPU sdx52_vel", cpu_tend-cpu_tstart); &
        total_cpu_marsh_time = total_cpu_marsh_time + (cpu_tend-cpu_tstart); end if
   call validate(sdx52_meta)
#endif
   nr=nr+1
...
#if USE_MPIX == 1
  call MPIX_ISEND_C(c_sdx52_id, ncout(nr), MPI_REAL8, &
                  neighb(1), 1021, comm_worker, reqX(nr), ierr)
#endif
   if(DETAILED_TIMING .eq. 1) then; call log_comm ("sdx52", neighb(1), ncout(nr)); end if
 endif
!-from 2 to 1
 if(neighb(2)>-1) then
   ...
   enddo
   enddo
   if(DETAILED_TIMING .eq. 1) then; call record_time(cpu_tend); call log_timing ("CPU sdx41_vel", cpu_tend-cpu_tstart);  &
        total_cpu_marsh_time = total_cpu_marsh_time + (cpu_tend-cpu_tstart); end if
   call validate(sdx41_meta)
#endif   
...
#if USE_MPIX == 1
     call MPIX_ISEND_C(c_sdx41_id, ncout(nr), MPI_REAL8, &
                  neighb(2), 2011, comm_worker, reqX(nr), ierr)
#endif
   if(DETAILED_TIMING .eq. 1) then; call log_comm ("sdx41", neighb(2), ncout(nr)); end if
   ...
   enddo
   enddo
   if(DETAILED_TIMING .eq. 1) then; call record_time(cpu_tend); call log_timing ("CPU sdx42_vel", cpu_tend-cpu_tstart);  &
        total_cpu_marsh_time = total_cpu_marsh_time + (cpu_tend-cpu_tstart); end if
   call validate(sdx42_meta)
#endif   
   nr=nr+1
...
#if USE_MPIX == 1
    call MPIX_ISEND_C(c_sdx42_id, ncout(nr), MPI_REAL8, &
                  neighb(2), 2021, comm_worker, reqX(nr), ierr)
#endif
   if(DETAILED_TIMING .eq. 1) then; call log_comm ("sdx42", neighb(2), ncout(nr)); end if
 endif
...
!-from 3 to 4
 if(neighb(3)>-1) then
   ...
   enddo
   enddo
   if(DETAILED_TIMING .eq. 1) then; call record_time(cpu_tend); call log_timing ("CPU sdy51_vel", cpu_tend-cpu_tstart);  &
        total_cpu_marsh_time = total_cpu_marsh_time + (cpu_tend-cpu_tstart); end if
   call validate(sdy51_meta)
#endif   
   nr=nr+1
...
#if USE_MPIX == 1
    call MPIX_ISEND_C(c_sdy51_id, ncout(nr), MPI_REAL8, &
                  neighb(3), 3011, comm_worker, reqX(nr), ierr)
#endif
   if(DETAILED_TIMING .eq. 1) then; call log_comm ("sdy51", neighb(3), ncout(nr)); end if
   ...
   enddo
   enddo
   if(DETAILED_TIMING .eq. 1) then; call record_time(cpu_tend); call log_timing ("CPU sdy52_vel", cpu_tend-cpu_tstart);  &
        total_cpu_marsh_time = total_cpu_marsh_time + (cpu_tend-cpu_tstart); end if
   call validate(sdy52_meta)
#endif 
...
#if USE_MPIX == 1
  call MPIX_ISEND_C(c_sdy52_id, ncout(nr), MPI_REAL8, &
                  neighb(3), 3021, comm_worker, reqX(nr), ierr)
#endif
   if(DETAILED_TIMING .eq. 1) then; call log_comm ("sdy52", neighb(3), ncout(nr)); end if
 endif
!-from 4 to 3
 if(neighb(4)>-1) then
   if(DETAILED_TIMING .eq. 1) then;  call record_time(gpu_tstart) ; end if
   ...
   enddo
   enddo
   if(DETAILED_TIMING .eq. 1) then; call record_time(cpu_tend); call log_timing ("CPU sdy41_vel", cpu_tend-cpu_tstart);  &
        total_cpu_marsh_time = total_cpu_marsh_time + (cpu_tend-cpu_tstart); end if
   call validate(sdy41_meta)
#endif
   nr=nr+1
...
#if USE_MPIX == 1
  call MPIX_ISEND_C(c_sdy41_id, ncout(nr), MPI_REAL8, &
                  neighb(4), 4011, comm_worker, reqX(nr), ierr)
#endif
   if(DETAILED_TIMING .eq. 1) then; call log_comm ("sdy41", neighb(4), ncout(nr)); end if
  ...
  if(DETAILED_TIMING .eq. 1) then; call record_time(gpu_tend); call log_timing ("GPU sdy42_vel", gpu_tend-gpu_tstart); &
   ...
   enddo
   enddo
   if(DETAILED_TIMING .eq. 1) then; call record_time(cpu_tend); call log_timing ("CPU sdy42_vel", cpu_tend-cpu_tstart);  &
        total_cpu_marsh_time = total_cpu_marsh_time + (cpu_tend-cpu_tstart); end if
   call validate(sdy42_meta)
#endif  
...
#if USE_MPIX == 1
  call MPIX_ISEND_C(c_sdy42_id, ncout(nr), MPI_REAL8, &
                  neighb(4), 4021, comm_worker, reqX(nr), ierr)
#endif
   if(DETAILED_TIMING .eq. 1) then; call log_comm ("sdy42", neighb(4), ncout(nr)); end if
...
#if USE_MPIX == 1
  call MPIX_IRECV_C(c_rcx51_id, ncout(nr), MPI_REAL8, &
                  neighb(2), 1011, comm_worker, reqX(nr), ierr)
#endif
   if(DETAILED_TIMING .eq. 1) then; call log_comm ("rcx51", neighb(2), ncout(nr)); end if
...
#if USE_MPIX == 1
      call MPIX_IRECV_C(c_rcx52_id, ncout(nr), MPI_REAL8, &
                  neighb(2), 1021, comm_worker, reqX(nr), ierr)
#endif
   if(DETAILED_TIMING .eq. 1) then; call log_comm ("rcx52", neighb(2), ncout(nr)); end if
 endif
!-from 2 to 1
 if(neighb(1)>-1) then
   nr=nr+1
...
#if USE_MPIX == 1
      call MPIX_IRECV_C(c_rcx41_id, ncout(nr), MPI_REAL8, &
                  neighb(1), 2011, comm_worker, reqX(nr), ierr)
     
!      call MPIX_RECV_C1(c_rcx41_id, ncout(nr), MPI_REAL8, &
!                  neighb(1), 2011, comm_worker, ierr)

! call MPI_Type_extent(MPI_REAL8, real_8_itemsize, ierror)
! size=4*nztop*nytop*real_8_itemsize
! call MPI_Alloc_mem(size, MPI_INFO_NULL, temp_rcx41C , ierr)
! call c_f_pointer(temp_rcx41C, rcx41_tmp, (/ nztop,nytop,4 /))
!
! call MPIX_IRECV_C1( temp_rcx41C, ncout(nr), MPI_REAL8, &
!                  neighb(1), 2011, comm_worker, req(nr), ierr)
! call MPI_WAITALL(nr,req,status,ierr)
!      write(*,*) rcx41_tmp

#endif
   if(DETAILED_TIMING .eq. 1) then; call log_comm ("rcx41", neighb(1), ncout(nr)); end if
...
#if USE_MPIX == 1
  call MPIX_IRECV_C(c_rcx42_id, ncout(nr), MPI_REAL8, &
                  neighb(1), 2021, comm_worker, reqX(nr), ierr)
#endif
   if(DETAILED_TIMING .eq. 1) then; call log_comm ("rcx42", neighb(1), ncout(nr)); end if
 endif
!-from 3 to 4
 if(neighb(4)>-1) then
   nr=nr+1
...
#if USE_MPIX == 1
  call MPIX_IRECV_C(c_rcy51_id, ncout(nr), MPI_REAL8, &
                  neighb(4), 3011, comm_worker, reqX(nr), ierr)
#endif
   if(DETAILED_TIMING .eq. 1) then; call log_comm ("rcy51", neighb(4), ncout(nr)); end if
...
#if USE_MPIX == 1
  call MPIX_IRECV_C(c_rcy52_id, ncout(nr), MPI_REAL8, &
                  neighb(4), 3021, comm_worker, reqX(nr), ierr)
#endif
   if(DETAILED_TIMING .eq. 1) then; call log_comm ("rcy52", neighb(4), ncout(nr)); end if
...
#if USE_MPIX == 1
      call MPIX_IRECV_C(c_rcy41_id, ncout(nr), MPI_REAL8, &
                  neighb(3), 4011, comm_worker, reqX(nr), ierr)
#endif
   if(DETAILED_TIMING .eq. 1) then; call log_comm ("rcy41", neighb(3), ncout(nr)); end if
...
#if USE_MPIX == 1
      call MPIX_IRECV_C(c_rcy42_id, ncout(nr), MPI_REAL8, &
                  neighb(3), 4021, comm_worker, reqX(nr), ierr)
#endif
   if(DETAILED_TIMING .eq. 1) then; call log_comm ("rcy42", neighb(3), ncout(nr)); end if
 endif
...
!-from 1 to 2
 if(neighb(2)>-1) then
   ...
   enddo
   enddo
   if(DETAILED_TIMING .eq. 1) then; call record_time(cpu_tend); call log_timing ("CPU rcx51_vel", cpu_tend-cpu_tstart);  &
        total_cpu_marsh_time = total_cpu_marsh_time + (cpu_tend-cpu_tstart); end if
   call validate(v1x_meta)
   call validate(v1y_meta)
   call validate(v1z_meta)
  ...
  do k=1,nzbtm
   ...
   enddo
   enddo
   if(DETAILED_TIMING .eq. 1) then; call record_time(cpu_tend); call log_timing ("CPU rcx52_vel", cpu_tend-cpu_tstart);  &
        total_cpu_marsh_time = total_cpu_marsh_time + (cpu_tend-cpu_tstart); end if
   call validate(v2x_meta)
   call validate(v2y_meta)
   call validate(v2z_meta)
...
!-from 2 to 1
 if(neighb(1)>-1) then
   ...
   enddo
   enddo
   if(DETAILED_TIMING .eq. 1) then; call record_time(cpu_tend); call log_timing ("CPU rcx41_vel", cpu_tend-cpu_tstart);  &
        total_cpu_marsh_time = total_cpu_marsh_time + (cpu_tend-cpu_tstart); end if
   call validate(v1x_meta)
   call validate(v1y_meta)
   call validate(v1z_meta)
   ...
   enddo
   enddo
   if(DETAILED_TIMING .eq. 1) then; call record_time(cpu_tend); call log_timing ("CPU rcx42_vel", cpu_tend-cpu_tstart);  &
        total_cpu_marsh_time = total_cpu_marsh_time + (cpu_tend-cpu_tstart); end if
   call validate(v2x_meta)
   call validate(v2y_meta)
   call validate(v2z_meta)
...
!-from 3 to 4
 if(neighb(4)>-1) then
  ...
  enddo
  enddo
   if(DETAILED_TIMING .eq. 1) then; call record_time(cpu_tend); call log_timing ("CPU rcy51_vel", cpu_tend-cpu_tstart);  &
        total_cpu_marsh_time = total_cpu_marsh_time + (cpu_tend-cpu_tstart); end if
   call validate(v1x_meta)
   call validate(v1y_meta)
   call validate(v1z_meta)
   ...
   enddo
   enddo
   if(DETAILED_TIMING .eq. 1) then; call record_time(cpu_tend); call log_timing ("CPU rcy52_vel", cpu_tend-cpu_tstart);  &
        total_cpu_marsh_time = total_cpu_marsh_time + (cpu_tend-cpu_tstart); end if
   call validate(v2x_meta)
   call validate(v2y_meta)
   call validate(v2z_meta)
...
!-from 4 to 3
 if(neighb(3)>-1) then
   ...
   enddo
   enddo
   if(DETAILED_TIMING .eq. 1) then; call record_time(cpu_tend); call log_timing ("CPU rcy41_vel", cpu_tend-cpu_tstart);  &
        total_cpu_marsh_time = total_cpu_marsh_time + (cpu_tend-cpu_tstart); end if
   call validate(v1x_meta)
   call validate(v1y_meta)
   call validate(v1z_meta)
   ...
   enddo
   enddo
   if(DETAILED_TIMING .eq. 1) then; call record_time(cpu_tend); call log_timing ("CPU rcy42_vel", cpu_tend-cpu_tstart);  &
        total_cpu_marsh_time = total_cpu_marsh_time + (cpu_tend-cpu_tstart); end if
   call validate(v2x_meta)
   call validate(v2y_meta)
   call validate(v2z_meta)
 ...
 if(DETAILED_TIMING .eq. 1) then; call log_timing ("GPU Marshaling ISEND_IRECV_Velocity", total_gpu_marsh_time); end if
#if VALIDATE_MODE == 1
 if(DETAILED_TIMING .eq. 1) then; call log_timing ("CPU Marshaling ISEND_IRECV_Velocity", total_cpu_marsh_time); end if
#endif
...
#if LOGGING_ENABLED == 1  
 call log_string("----------SUBROUTINE velocity_interp")
#endif
 total_gpu_marsh_time =0.0
 total_cpu_marsh_time =0.0
...
!-from 1 to 2
 if(neighb(1)>-1) then
  ...
  sdx1(j0+4) = v2z(1,2,ny2p2)
   if(DETAILED_TIMING .eq. 1) then; call record_time(cpu_tend); call log_timing ("CPU sdx1_vel", cpu_tend-cpu_tstart);  &
        total_cpu_marsh_time = total_cpu_marsh_time + (cpu_tend-cpu_tstart); end if
  call validate(sdx1_meta)
#endif
   nr=nr+1
...
#if USE_MPIX == 0 || VALIDATE_MODE == 1
    call MPI_ISEND(sdx1(1), ncout(nr), MPI_REAL8, &
                  neighb(1), 11, comm_worker, req(nr), ierr)
#endif                  
#if USE_MPIX == 1
   call MPIX_ISEND_C(c_sdx1_id, ncout(nr), MPI_REAL8, &
                  neighb(1), 111, comm_worker, reqX(nr), ierr)
#endif 
   if(DETAILED_TIMING .eq. 1) then; call log_comm ("sdx1", neighb(1), ncout(nr)); end if
 endif
!-from 3 to 4
 if(neighb(3)>-1) then
   if(DETAILED_TIMING .eq. 1) then;  call record_time(gpu_tstart) ; end if
   ...
   if(DETAILED_TIMING .eq. 1) then; call record_time(cpu_tend); call log_timing ("CPU sdy1_vel", cpu_tend-cpu_tstart);  &
        total_cpu_marsh_time = total_cpu_marsh_time + (cpu_tend-cpu_tstart); end if
   call validate(sdy1_meta)
#endif  
...
#if USE_MPIX == 0 || VALIDATE_MODE == 1
   call MPI_ISEND(sdy1(1), ncout(nr), MPI_REAL8, &
          neighb(3), 33, comm_worker, req(nr), ierr)
#endif 
#if USE_MPIX == 1
    call MPIX_ISEND_C(c_sdy1_id, ncout(nr), MPI_REAL8, &
          neighb(3), 331, comm_worker, reqX(nr), ierr)
#endif 
   if(DETAILED_TIMING .eq. 1) then; call log_comm ("sdy1", neighb(3), ncout(nr)); end if
 endif
!-from 2 to 1 
 if(neighb(2)>-1) then
   if(DETAILED_TIMING .eq. 1) then;  call record_time(gpu_tstart) ; end if
   ...
   if(DETAILED_TIMING .eq. 1) then; call record_time(cpu_tend); call log_timing ("CPU sdx2_vel", cpu_tend-cpu_tstart);  &
        total_cpu_marsh_time = total_cpu_marsh_time + (cpu_tend-cpu_tstart); end if
   call validate(sdx2_meta)
#endif  
   ...
   ncout(nr) = j0 + 7
#if USE_MPIX == 0 || VALIDATE_MODE == 1
    call MPI_ISEND(sdx2(1), ncout(nr), MPI_REAL8, &
                  neighb(2), 22, comm_worker, req(nr), ierr)
#endif 
#if USE_MPIX == 1
   call MPIX_ISEND_C(c_sdx2_id, ncout(nr), MPI_REAL8, &
                  neighb(2), 221, comm_worker, reqX(nr), ierr)
#endif 
   if(DETAILED_TIMING .eq. 1) then; call log_comm ("sdx2", neighb(2), ncout(nr)); end if
 endif
!-from 4 to 3 
 if(neighb(4)>-1) then
   if(DETAILED_TIMING .eq. 1) then;  call record_time(gpu_tstart) ; end if
   ...
   if(DETAILED_TIMING .eq. 1) then; call record_time(cpu_tend); call log_timing ("CPU sdy2_vel", cpu_tend-cpu_tstart);  &
        total_cpu_marsh_time = total_cpu_marsh_time + (cpu_tend-cpu_tstart); end if
   call validate(sdy2_meta)
#endif   
   nr=nr+1
...
#if USE_MPIX == 0 || VALIDATE_MODE == 1
  call MPI_ISEND(sdy2(1), ncout(nr), MPI_REAL8, &
                  neighb(4), 44, comm_worker, req(nr), ierr)
#endif 
#if USE_MPIX == 1
   call MPIX_ISEND_C(c_sdy2_id, ncout(nr), MPI_REAL8, &
                  neighb(4), 441, comm_worker, reqX(nr), ierr)
#endif 
   if(DETAILED_TIMING .eq. 1) then; call log_comm ("sdy2", neighb(4), ncout(nr)); end if
 endif
...
#if USE_MPIX == 0 || VALIDATE_MODE == 1
   call MPI_IRECV(rcx2(1), ncout(nr), MPI_REAL8, &
    neighb(2), 11, comm_worker, req(nr), ierr)
#endif
#if USE_MPIX == 1
   call MPIX_IRECV_C(c_rcx2_id, ncout(nr), MPI_REAL8, &
    neighb(2), 111, comm_worker, reqX(nr), ierr)
#endif
   if(DETAILED_TIMING .eq. 1) then; call log_comm ("rcx2", neighb(2), ncout(nr)); end if
 endif
!-from 3 to 4
 if(neighb(4)>-1) then
   nr=nr+1
...
#if USE_MPIX == 0 || VALIDATE_MODE == 1
   call MPI_IRECV(rcy2(1), ncout(nr), MPI_REAL8, &
                  neighb(4), 33, comm_worker, req(nr), ierr)
#endif   
#if USE_MPIX == 1
   call MPIX_IRECV_C(c_rcy2_id, ncout(nr), MPI_REAL8, &
                  neighb(4), 331, comm_worker, reqX(nr), ierr)
#endif   
   if(DETAILED_TIMING .eq. 1) then; call log_comm ("rcy2", neighb(4), ncout(nr)); end if
 endif
...
!-from 2 to 1 
 if(neighb(1)>-1) then
   ...
   ncout(nr) = ny2p2+1 + 7
#if USE_MPIX == 0 || VALIDATE_MODE == 1
   call MPI_IRECV(rcx1(1), ncout(nr), MPI_REAL8, &
                  neighb(1), 22, comm_worker, req(nr), ierr)
#endif                  
#if USE_MPIX == 1
   call MPIX_IRECV_C(c_rcx1_id, ncout(nr), MPI_REAL8, &
                  neighb(1), 221, comm_worker, reqX(nr), ierr)
#endif   
   if(DETAILED_TIMING .eq. 1) then; call log_comm ("rcx1", neighb(1), ncout(nr)); end if
endif
...
#if USE_MPIX == 0 || VALIDATE_MODE == 1
   call MPI_IRECV(rcy1(1), ncout(nr), MPI_REAL8, &
                  neighb(3), 44, comm_worker, req(nr), ierr)
#endif 
#if USE_MPIX == 1
   call MPIX_IRECV_C(c_rcy1_id, ncout(nr), MPI_REAL8, &
                  neighb(3), 441, comm_worker, reqX(nr), ierr)
#endif 
   if(DETAILED_TIMING .eq. 1) then; call log_comm ("rcy1", neighb(3), ncout(nr)); end if
...
!-from 1 to 2
 if(neighb(2)>-1) then
   ...
   if(DETAILED_TIMING .eq. 1) then; call record_time(cpu_tend); call log_timing ("CPU rcx2_vel", cpu_tend-cpu_tstart);  &
        total_cpu_marsh_time = total_cpu_marsh_time + (cpu_tend-cpu_tstart); end if
   call validate(v2z_meta)
#endif   
 endif
!-from 3 to 4
 if(neighb(4)>-1) then
   ...
   if(DETAILED_TIMING .eq. 1) then; call record_time(cpu_tend); call log_timing ("CPU rcy2_vel", cpu_tend-cpu_tstart);  &
        total_cpu_marsh_time = total_cpu_marsh_time + (cpu_tend-cpu_tstart); end if
   call validate(v2y_meta)
   call validate(v2x_meta)
#endif   
...
!-from 2 to 1
 if(neighb(1)>-1) then
   ...
   if(DETAILED_TIMING .eq. 1) then; call record_time(cpu_tend); call log_timing ("CPU rcx1_vel", cpu_tend-cpu_tstart);  &
        total_cpu_marsh_time = total_cpu_marsh_time + (cpu_tend-cpu_tstart); end if
   call validate(v2x_meta)
   call validate(v2y_meta)
   call validate(v2z_meta)
...
!-from 4 to 3
 if(neighb(3)>-1) then
   ...
   if(DETAILED_TIMING .eq. 1) then; call record_time(cpu_tend); call log_timing ("CPU rcy1_vel", cpu_tend-cpu_tstart);  &
        total_cpu_marsh_time = total_cpu_marsh_time + (cpu_tend-cpu_tstart); end if
   call validate(v2x_meta)
   call validate(v2y_meta)
   call validate(v2z_meta)
 ...
 if(DETAILED_TIMING .eq. 1) then; call log_timing ("GPU Marshaling velocity_interp", total_gpu_marsh_time); end if
#if VALIDATE_MODE == 1
 if(DETAILED_TIMING .eq. 1) then; call log_timing ("CPU Marshaling velocity_interp", total_cpu_marsh_time); end if
#endif

 call record_time(tstart)
 call interpl_3vbtm
 call record_time(tend)
 call log_timing("interpl_3vbtm", tend-tstart)
 return
...
subroutine interpl_3vbtm
 ...
 double precision::vv1,vv2,vv3,vv4 
 real(c_double)::gpu_tstart, gpu_tend, cpu_tstart, cpu_tend
 real(c_double):: total_gpu_marsh_time=0.0, total_cpu_marsh_time=0.0
 total_gpu_marsh_time =0.0
 total_cpu_marsh_time =0.0
...
#if LOGGING_ENABLED == 1  
 call log_string("----------SUBROUTINE interpl_3vbtm")
#endif
...
!---Vx----
 ...
 do jj=0,nyvx
   ...
   j=min0(jj*3+1,ny1p2)
 enddo
 if(DETAILED_TIMING .eq. 1) then; call record_time(cpu_tend); call log_timing ("CPU interpl_3vbtm_1", cpu_tend-cpu_tstart);  &
        total_cpu_marsh_time = total_cpu_marsh_time + (cpu_tend-cpu_tstart); end if
...
do i=1,nxtop
  ...
  enddo
enddo
if(DETAILED_TIMING .eq. 1) then; call record_time(cpu_tend); call log_timing ("CPU interpl_3vbtm_3", cpu_tend-cpu_tstart);  &
        total_cpu_marsh_time = total_cpu_marsh_time + (cpu_tend-cpu_tstart); end if
! ---Vy----
i0=nx2p2+2
i=0
...
do ii=0,nxvy
  ...
  i=min0(ii*3+1,nx1p2)
enddo
if(DETAILED_TIMING .eq. 1) then; call record_time(cpu_tend); call log_timing ("CPU interpl_3vbtm_6", cpu_tend-cpu_tstart);  &
        total_cpu_marsh_time = total_cpu_marsh_time + (cpu_tend-cpu_tstart); end if
...
if(neighb(1) > -1) then
  ...
  if(DETAILED_TIMING .eq. 1) then; call record_time(cpu_tend); call log_timing ("CPU interpl_3vbtm_7", cpu_tend-cpu_tstart);  &
        total_cpu_marsh_time = total_cpu_marsh_time + (cpu_tend-cpu_tstart); end if
endif
...
if(neighb(2) > -1) then
  ...
  if(DETAILED_TIMING .eq. 1) then; call record_time(cpu_tend); call log_timing ("CPU interpl_3vbtm_8", cpu_tend-cpu_tstart);  &
        total_cpu_marsh_time = total_cpu_marsh_time + (cpu_tend-cpu_tstart); end if
endif
i=0
...
do ii=0,nxvy
  ...
  i=min0(ii*3+1,nx1p2)
enddo
if(DETAILED_TIMING .eq. 1) then; call record_time(cpu_tend); call log_timing ("CPU interpl_3vbtm_9", cpu_tend-cpu_tstart);  &
        total_cpu_marsh_time = total_cpu_marsh_time + (cpu_tend-cpu_tstart); end if
!
!
if(DETAILED_TIMING .eq. 1) then;  call record_time(cpu_tstart) ; end if
...
enddo
if(DETAILED_TIMING .eq. 1) then; call record_time(cpu_tend); call log_timing ("CPU interpl_3vbtm_11", cpu_tend-cpu_tstart);  &
        total_cpu_marsh_time = total_cpu_marsh_time + (cpu_tend-cpu_tstart); end if
! ---Region II---
...
do jj=1,nybtm
  ...
  enddo
enddo
if(DETAILED_TIMING .eq. 1) then; call record_time(cpu_tend); call log_timing ("CPU interpl_3vbtm_13", cpu_tend-cpu_tstart);  &
        total_cpu_marsh_time = total_cpu_marsh_time + (cpu_tend-cpu_tstart); end if
...
do jj=1,nybtm
  ...
  enddo
enddo
if(DETAILED_TIMING .eq. 1) then; call record_time(cpu_tend); call log_timing ("CPU interpl_3vbtm_14", cpu_tend-cpu_tstart);  &
        total_cpu_marsh_time = total_cpu_marsh_time + (cpu_tend-cpu_tstart); end if
...
do jj=1,nybtm
  ...
  enddo
enddo
if(DETAILED_TIMING .eq. 1) then; call record_time(cpu_tend); call log_timing ("CPU interpl_3vbtm_15", cpu_tend-cpu_tstart);  &
        total_cpu_marsh_time = total_cpu_marsh_time + (cpu_tend-cpu_tstart); end if
 call validate(v1x_meta)
 call validate(v1y_meta)
 call validate(v1z_meta)
...
#if VALIDATE_MODE == 1
 if(DETAILED_TIMING .eq. 1) then; call log_timing ("CPU Marshaling interpl_3vbtm", total_cpu_marsh_time); end if
#endif
...
FUNCTION gammln(xx)
 ...
 double precision:: cof(6), ser,tmp,x,y, stp=2.5066282746310005d0
...
subroutine ini_plane_wave(myid,dt)
 ...
 double precision:: psvsh_vector(3,3),rvps(3)
 double precision:: pi,st,dp,csst,snst,csdp,sndp,dt05
 double precision:: xs,ys,zs,x0,y0,z0,dsdx,dsdy,dsdz,rstim,cfamp,strike,dip
 double precision:: vxmm,vymm,vzmm,uxmm,uymm,uzmm,tpmm,t1mm,t2mm,t3mm,t4mm,t5mm,t6mm
 double precision:: v11,v22,xi,yi,zi,xhf,yhf,zhf,smu,clm,pl2m,rv,xr,yr,zr
 double precision:: tpw,tpx,tpy,tpz,ux,uy,uz,trav,taper,arg,vel_plan
!
 pi=atan(1.0)*4.0
 nfadd=0
 dt05=0.5*dt
...
function vel_plan(t,rstim)
 implicit NONE
 double precision, intent(IN):: t,rstim
 double precision:: arg,vel_plan
...
subroutine print_3d_array(array_name, array_ptr, size_x, size_y, size_z)
    ...
    do j=1,size_y
    do i=1,size_x
        write(*,"(f8.3)", advance='no') array_ptr(i,j,k)
    end do
        write(*,*) " "
    end do
        write(*,*) "end"
